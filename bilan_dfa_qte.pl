...
while (($es_no_do,$date)=$sth->fetchrow_array){
	...
	$montant=&get("select sum(livb_qte_ent) from dfc.livraison_b where livb_id='$enh_document'");
...
while (($livh_four)=$sth->fetchrow_array){
	...
	$ca=&get("select sum(qte) from corsica.panier_caisse,corsica.ticket_caisse,corsica.produit where date>='$premiere' and date<'$derniere'  and vendeuse!='sylvain' and ticket_date=date and ticket_vendeuse=vendeuse and ticket_pdv=pdv and no_cde=ticket_no and ticket_sup=0 and code=pr_cd_pr and pr_four=$livh_four ","af");
	$stock=$montant-$ca;
	print "<tr>";
