...
sub run(){
	...
	for ($mois=1;$mois<=$maxi;$mois++){
		$ca+=&get("select sum(ca_total) from $client.vol,$client.caissesql where v_code=ca_code and v_rot=ca_rot and year(v_date_sql)=year(curdate()) and month(v_date_sql)='$mois'");
		$achat+=&get("select sum(es_qte*pr_prac/10000) from enso_$client where year(es_dt)=year(curdate()) and month(es_dt)='$mois'");
		$q=43*$ca/100;
		print "['$mois',$achat,$ca,$q],";
	}	
