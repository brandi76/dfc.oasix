...
$desi_court=$html->param("desi_court");
$desi_tpe=substr($html->param("desi_tpe"),0,15);
...
$pr_four_pub=$html->param("pr_four_pub");

if ($action eq "sup") {
    #$forcer dans la barre d'adsresse
	foreach $client (@bases_client) {
	  &save ("delete from $client.produit  where pr_cd_pr='$pr_cd_pr'","aff");
	  &save ("delete from $client.carton  where car_cd_pr='$pr_cd_pr'","aff");
	  &save ("delete from $client.produit_plus where pr_cd_pr='$pr_cd_pr'","aff");
	}
	 &save ("delete from dfc.produit_plus where pr_cd_pr='$pr_cd_pr'","aff");
	$action="";
}
...
if (($action eq "creation") or ($action eq "clone")){
	...
	{	
	  foreach $client (@bases_client) {
	    $desi_nouveau=&get("select pr_desi from produit where pr_cd_pr='$pr_cd_pr'");
	    $desi_nouveau="new".$desi_nouveau;
	    &save ("INSERT ignore into $client.produit  select '$nouveau','$desi_nouveau','0','0','0',pr_douane,pr_ventil,'0',pr_type,pr_prx_vte,'0',pr_sup,pr_emb,'0',pr_deg,pr_pdn,'0',pr_acquit,pr_orig,pr_pdb,pr_qte_comp,pr_cond,pr_devac,pr_four,'','' from dfc.produit where pr_cd_pr='$pr_cd_pr'","af");
	    &save ("INSERT ignore into $client.carton select '$nouveau', car_carton, car_pal from dfc.carton where car_cd_pr='$pr_cd_pr'","af");
	    &save ("INSERT ignore into $client.produit_plus select '$nouveau',curdate(),'','','','','','','','',pr_fragrance,pr_vapo,'',pr_famille,pr_four_pub from dfc.produit_plus where pr_cd_pr='$pr_cd_pr'","af");
	  } 
	   &save ("INSERT ignore into dfc.produit_desi select '$nouveau',desi_court,desi_tpe,marque,marque_court from dfc.produit_desi where code='$pr_cd_pr'","af");
...
if ($action eq "modif"){
	if ($logistique eq "on"){
	  foreach $client (@bases_client) {
# 		&save("update $client.produit set pr_desi='$pr_desi',pr_sup='$pr_sup',pr_codebarre='$pr_codebarre',pr_type='$pr_type' where pr_cd_pr='$pr_cd_pr'");
 		&save("update $client.produit set pr_sup='$pr_sup',pr_desi='$pr_desi',pr_pdn='$pr_pdn' where pr_cd_pr='$pr_cd_pr'");
		&save("replace into $client.carton value ('$pr_cd_pr','$car_carton','$car_pal')");
  		&save("update $client.produit_plus set pr_famille='$pr_famille',pr_fragrance='$pr_fragrance' where pr_cd_pr='$pr_cd_pr'");
	  }
	  &save("replace into produit_desi values ('$pr_cd_pr','$desi_court','$desi_tpe','$marque','')");
	}
	if ($achat eq "on"){
		$pr_prx_rev*=100;
		$pr_prx_vte*=100;
		$pr_prac*=100;
		foreach $client (@bases_client) {
		  &save("update $client.produit set pr_prac='$pr_prac',pr_prx_rev='$pr_prx_rev',pr_four='$pr_four',pr_refour='$pr_refour',pr_prx_vte='$pr_prx_vte' where pr_cd_pr='$pr_cd_pr'","af");
		  &save("insert ignore into $client.prixachat value ('$pr_cd_pr',curdate(),'$pr_prac')");
# 		  if ($prixvauto eq "on"){
...
# 		  }
	...
	if ($douane eq "on"){
		$pr_deg*=100;
		foreach $client (@bases_client) {
		  &save("update $client.produit set pr_douane='$pr_douane',pr_ventil='$pr_ventil',pr_deg='$pr_deg',pr_pdn='$pr_pdn',pr_pdb='$pr_pdb' where pr_cd_pr='$pr_cd_pr'");
		}  
	}
	if ($commercial eq "on"){
	    foreach $client (@bases_client) {
	      &save("replace into $client.produit_plus values('$pr_cd_pr','$pr_date_creation',now(),'$pr_nom','$pr_newflag','$pr_saison','$pr_impose','$pr_remplace','$pr_date_deb','$pr_date_fin','$pr_fragrance','$pr_vapo','$pr_remise_com','$pr_famille','$pr_four_pub')","af");
	      &save("update $client.produit set pr_pdn='$pr_pdn' where pr_cd_pr='$pr_cd_pr'");
	    }
	}
	$action="visu";
}
...
# 	&save("replace into mouchard values ($pr_cd_pr,now())");
	...
	$query="select * from produit_desi where code='$pr_cd_pr'";
	$sth=$dbh->prepare($query);
	$sth->execute();
...
# 	print " Code couleur </a><br><br>";
	...
	{
		$query="select produit.pr_cd_pr,pr_desi,pr_sup,pr_stre from produit,mouchard where produit.pr_cd_pr=mouchard.pr_cd_pr group by mouchard.pr_cd_pr order by mouchard.date desc";
	}
	$action="";
	$sth=$dbh->prepare($query);
	$sth->execute();
...
if ($action eq "visu"){
	...
	print "<center>";
	if ($lock eq "loc"){print " <div style=background-color:pink>";}else{"<div class=titre>";}
	print "$pr_cd_pr  $pr_desi ";
...
# print "<img src=\"http://ibs.oasix.fr/code_barre/html/image.php?code=ean13&o=1&t=30&r=1&text=$code_barre&f=2&a1=&a2\" align=middle>";
	...
	print "<br><span style=background:lavender>Désormais les nouveaux produits doivent être créé prioritairement par fichier->produit->creation</span><br>";
	print "<input type=submit name=action value=creation> <input type=submit name=action value=clone> </form>";
...
sub table_douane {
	my($option)=$_[0];
	if (($lock eq "loc")&&($option==1)){&info("Produit non harmonisé entre les bases modification desactivée");$option=0;	}
...
sub table_achat {
	my($option)=$_[0];
	if (($option==1)&&($lock eq "loc")){&info("Produit non harmonisé entre les bases modification desactivée");$option=0;	}
...
sub table_logistique {
	...
	print "<td><input type=text size=50 $readonly name=pr_desi value='$pr_desi' style=\"background-color:$color;text-align:left;\"></td></tr>";
	print "<tr><td width=210>Désignation Commerciale</td>";
	print "<td><input type=text $readonly name=desi_court value='$desi_court' style=\"background-color:$color;text-align:left;\"></td></tr>";
	print "<tr><td width=210>Désignation Tpe</td>";
	print "<td><input type=text $readonly name=desi_tpe value='$desi_tpe' style=\"background-color:$color;;text-align:left;\"></td></tr>";
	print "<tr><td width=210>Marque</td><td>";
	if ($option==0){print "<input type=text $readonly name=marque value='$marque' style=\"background-color:$color;;text-align:left;\">";}
	else {
	print "<input list=marque type=text name=marque value='$marque' style=text-align:left>";
	
	  print "<datalist id=marque>";
	  # print "<select name=marque style=;text-align:left;>";
	...
	if ($option==0){print "<input type=text readonly value='$table_famille{$pr_famille}' style=\"background-color:$color;\">";}
	else {
	...
	print "</td></tr>";
	@liste=("EDT eau de toilette","EDP eau de parfum","EDC eau de cologne","P parfum","EF eau fraiche","SDP soie de parfum","ET eau tonique","C coffret mini");
        $etat=$liste[$pr_fragrance];
	print "<tr><td>Type de Fragrance</td><td>";
 	if ($option==0){print "<input type=text $readonly name=pr_fragrance value='$etat' style=\"background-color:$color;\">";}
 	else {
 		print "<select name=pr_fragrance>";
 		for (my($i)=0;$i<=$#liste;$i++){
 			print "<option value=$i";
 			if ($pr_fragrance==$i){print " selected";}
 			print ">$liste[$i]</option><br>";
 		}
 		print "</select>";
 	}
	print "</td></tr>";
...
# 	print "<tr><td width=210>Famille</td><td>";
# 	if ($option==0){print "<input type=text readonly name=pr_type value='$pr_type $ventil' style=\"background-color:$color;\">";}
# 	else {
...
sub table_commer {
	...
	print "</td></tr>";
        print "<tr><td width=210>Date de fin </td><td>";
	...
	print "</td></tr>";
	@liste=("eau de toilette","eau de parfum","eau de cologne","parfum","eau fraiche","soie de parfum","eau tonique","coffret mini");
        $etat=$liste[$pr_fragrance];
	# &set_color(1);
	print "<tr><td>Type de Fragrance</td><td>";
 	if ($option==0){
	...
	print "</td></tr>";
 	...
 	print "<tr><td>Fournisseur Facture pub</td><td>";
	print "<input type=text $readonly name=pr_four_pub value='$pr_four_pub' style=\"background-color:$color;\">";
	# &set_color(1,$option);
	print "</td></tr>";
