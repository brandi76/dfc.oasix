...
$sth=$dbh->prepare($query);
	...
	while (($pr_cd_pr,$pr_desi,$pr_prac)=$sth->fetchrow_array)
	{
		...
		$total_gen+=$total;
		print  "<tr><td>$pr_cd_pr</td><td>$pr_desi</td><td align=right>$stck</td><td align=right>$pr_prac</td><td align=right>";
...
sub journal {
	...
	print MAIL   `date`;
	print MAIL   "<br><br><br><table border=1 cellspacing=0 ><tr><td>Solde en debut de journée (j-1)</td><td align=right>$stock</td></tr>\n";
