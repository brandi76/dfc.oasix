...
print &prac(3145891244601);
exit;
print "<center>Marge Navire base vente neptune , prix achat dernier prix ibs (parfums) ou neptune du mois (autres)<br>";

$query="select distinct vdu_navire from vendu_corsica_mois where vdu_mois>=801 and vdu_mois<=809 and vdu_navire!='0' and vdu_navire='MEGA 1'";
...
$query="select distinct vdu_mois from vendu_corsica_mois where vdu_mois >=801 and vdu_mois<=801 order by vdu_mois";
$sth=$dbh->prepare($query);
$sth->execute();
...
foreach $navire (@navire) {
	...
	foreach $mois (@mois) {
		...
		while (($pr_four,$pr_cd_pr,$pr_desi,$qte,$vendu,$achat)=$sth->fetchrow_array)
		{
			...
			if ($pr_cd_pr==3145891244601){
	  		 print "$pr_prac<br>";
	  		 }
	
			if ($pr_prac==0){$pr_prac=$achat;$nep=1;}
# 			print "$pr_cd_pr;$vendu;$pr_prac;$qte<br>";
			$achat=$qte*$pr_prac;
			if ($achat==0){next;}  
