...
if (($today>=13233)&&($today<=13450)){$decalheure=2;}   # ETE 2006
...
sub table{
	...
	while (($fl_date,$fl_vol,$fl_cd_cl,$fl_nbrot,$fl_troltype,$fl_apcode,$fl_part,$fl_nolot)=$sth->fetchrow_array){
		...
		if ($fl_apcode >0){
			$v_cd_cl=&get("select v_cd_cl from vol where v_code='$fl_apcode' and v_rot=1","af");
			if ($v_cd_cl != $fl_cd_cl){
		...
		print  "<td align=center><div class=ombre>$fl_vol</td><td align=center><div class=ombre>$fl_troltype<br><nobr>$lot_conteneur</td><td ";
		...
		print "<input type=button class=bouton value=modif onclick=\"";
		if ($fl_apcode != 0){
