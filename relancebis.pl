...
open(FILE2,"/home/var/spool/uucppublic/echeanc.txt");
@echeancier_dat=<FILE2>;
close(FILE2);

# fichier contenant les regroupements
%relance_idx = &get_index_num("relancebis",0);
open(FILE2,"/home/var/spool/uucppublic/relancebis.txt");     
@relance_dat = <FILE2>;
close (FILE2);
...
if ($niveau eq "rappel1"){$ref=&nbjour($date)-384;}
if ($niveau eq "rappel2"){$ref=&nbjour($date)-35;}
if ($niveau eq "rappel3"){$ref=&nbjour($date)-42;}
if ($niveau eq "rappel4"){$ref=&nbjour($date)-56;}
if ($niveau eq "rappel5"){$ref=&nbjour($date)-70;}
...
if ($action eq "telph") {
	...
	foreach (@table){
		...
		if ($html->param($facture) eq "on"){
			...
			&ajoute_n("/home/var/spool/uucppublic/relance-3bis.txt",$ligne,1);
		}
	}
	$action="rappel3";
}
...
sub selection {
if ($niveau eq "rappel1"){&tete("Premier rappel","/home/var/spool/uucppublic/echeanc.txt");}
if ($niveau eq "rappel2"){&tete("Deuxieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
if ($niveau eq "rappel3"){&tete("Troisieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
if ($niveau eq "rappel4"){&tete("Quatrieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
if ($niveau eq "rappel5"){&tete("Cinquieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
...
@tab=sort (@tab);
print "<form name=modif action=relancebis.pl>";
...
foreach (@tab){
	...
	$diff=$lien%6;
	$ec_no_fact+=0;
	$restetemptest=$ec_mont-$ec_reg;
	#ne pa afficher les - de 2€
	#if($restetemptest<2 && $restetemptest>=0){next;}
	if($restetemptest>-1.5 && $restetemptest<1.5){next;}
	...
	if ($rappel1_idx{$ec_no_fact}ne""){
		...
		print "<td align=center><img src=http://intranet.dom/relance1.gif border=0 align=top height=20><br><font size=-2>".date($date_1)."</td>";
	}
	else {
		print "<td>&nbsp;</td>";
	}
...
if ($niveau eq "rappel1"){
	print "<br><form action=relancebis.pl><center>";
...
if ($niveau eq "rappel5"){
	...
	print "<input type=hidden name=niveau value=$niveau>";
	print "<input type=hidden name=code_client value=$code_client>";
	print "<input type=submit value=\"Creer les lettres de Cinquième Relance\"></form>";
}
...
if ($niveau eq "rappel1"){$file="/home/var/spool/uucppublic/relance-1bis.txt";}
if ($niveau eq "rappel2"){$file="/home/var/spool/uucppublic/relance-2bis.txt";}
if ($niveau eq "rappel3"){
	print "<form name=telph action=relancebis.pl>";
	print "<input type=hidden name=niveau value=rappel3>";
	print "<input type=hidden name=code_client value=$code_client>";
	print "<input type=hidden name=action value=telph>";
}
if ($niveau eq "rappel3"){$file="/home/var/spool/uucppublic/relance-3bis.txt";}
if ($niveau eq "rappel4"){$file="/home/var/spool/uucppublic/relance-4bis.txt";}
if ($niveau eq "rappel5"){$file="/home/var/spool/uucppublic/relance-5bis.txt";}
$saut=$lignea=0;
$page=-1;
$page2=-12;
foreach $cle (keys(%afaire)) {
	...
	foreach (@table){
		($lien,$ec_cd_cl,$ec_no_fact,$ec_nom,$ec_dt,$ec_mont,$ec_reg,$ec_dt_reg,$ec_mont_dev,$ec_cd_dev)  = split(/;/,$_);
   		print "
	        <SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left:1cm;\">$ec_no_fact</span>
	        <SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left:3cm;\">$ec_dt</span>";
		($nul,$nul,$nul,$nul,$nul,$nul,$dev)=&selecte("/home/var/spool/uucppublic/pays.txt",$ec_cd_dev,0);
			
                print "<SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left:5cm;\">$ec_mont € ";
		...
		# Somme déjà régler . . .
	        if( $ec_reg !=0){
			$reg_flag="oui";
	               	print "<SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left:10cm;\">**$ec_reg €</span>";
	        }
	     	$ec_mont-=$ec_reg;
	        print "<SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left:13cm;\">";
	        print &separateur($ec_mont);
	        print " €</span>";
		...
		if ($niveau eq "rappel3"){
			 print "<SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left:16cm;\"><input type=checkbox name=$ec_no_fact></span>";
		}
 		$lignea+=0.5;
 		if($lignea == 3){
 			#sautepage();
 			...
 			if ($niveau eq "rappel1"){$page = $page+1;			&tete1();}
			if ($niveau eq "rappel2"){print "<DIV align=justify style=\"position:absolute;width:16cm;height:200px;top:",$page*27.7+18.5,"cm;left:1cm;border-style:solid;border-width:0px;border-color:#FFCC66;\">Suite page suivante . . .</DIV>";$page = $page+1;			&papier_entete();&tete2();}
			if ($niveau eq "rappel3"){$page = $page+1;			&tete3();}
			if ($niveau eq "rappel4"){$page = $page+1;			&tete4();}
			if ($niveau eq "rappel5"){$page = $page+1;			&tete5();}
 			$page2=-12;
# 		}else{
#			if ($niveau eq "rappel2" && $lignea<3.5){print "<DIV align=justify style=\"position:absolute;width:16cm;height:200px;top:",$page*27.7+18.5,"cm;left:1cm;border-style:solid;border-width:0px;border-color:#FFCC66;\">Suite page suivante . . .</DIV>";}
		}
		if($lignea == 3){
						$lignea=0;
			}	       
...
sub tete1 {
	...
	<DIV align=justify style=\"position:absolute;width:16cm;height:200px;top:",$page*27.7+10,"cm;left:1cm;border-style:solid;border-width:0px;border-color:#FFCC66;\">$lang[3]</DIV>
	<SPAN style=\"position: absolute; top: ",$page*27.7+12.5,"cm; left: 1cm;\"><U>RELEVE DE COMPTE</U></span>
...
sub pied2 {
	print "<SPAN style=\"position: absolute; top: ",$page*27.7+13.5+$lignea,"cm; left: 13cm;\"><b>";
	print &separateur($total);
	print " € ";
#	if ($reg_flag eq "non"){ # si aucun reglement n'a ete fait on affiche le total en devise
#		foreach $cle (keys(%total)){
...
#	}
	...
	<DIV align=justify style=\"position:absolute;width:16cm;height:200px;top:",$page*27.7+17,"cm;left:1cm;border-style:solid;border-width:0px;border-color:#FFCC66;\">$lang[9]</DIV>
	
	<SPAN style=\"position: absolute; top: ",$page*27.7+24.5,"cm; left: 10cm;\">$lang[10]</span>
...
sub tete5 {
	...
	<DIV align=justify style=\"position:absolute;width:16.5cm;height:200px;top:",$page*27.7+11.5,"cm;left:1cm;border-style:solid;border-width:0px;border-color:#FFCC66;\">$lang[3]</DIV>
	"
...
sub listedesclients {
	if ($action eq "rappel1"){&tete("Premier rappel","/home/var/spool/uucppublic/echeanc.txt");}
	if ($action eq "rappel2"){&tete("Deuxieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
	if ($action eq "rappel3"){&tete("Troisieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
	if ($action eq "rappel4"){&tete("Quatrieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
	if ($action eq "rappel5"){&tete("Cinquieme rappel ","/home/var/spool/uucppublic/echeanc.txt"); }
	...
	foreach (@echeancier_dat) {
		...
		if ((&nbjour($ec_dt)<=$ref)&&(&testsiauto() eq "oui")){
			...
			print "</td></tr>";
			$total+=$reste;
			#Suppression alex le 21/02/2002 -> relance affectuer: ecriture dans le fichier lors affichage liste clients#if ($action eq "rappel4"){
			#	$file="/home/var/spool/uucppublic/relance-4bis.txt";
			#	$ligne=$ec_cd_cl.";".$ec_no_fact.";".$date.";";
			#	&ajoute_n("$file",$ligne,1);
			#}
			#if ($action eq "rappel5"){
			#	$file="/home/var/spool/uucppublic/relance-5bis.txt";
			#	$ligne=$ec_cd_cl.";".$ec_no_fact.";".$date.";";
			#	&ajoute_n("$file",$ligne,1);
			#}
		}
...
sub html {
        &tete("Relance ","/home/var/spool/uucppublic/echeanc.txt"); 

	print <<"eof";
	<center><br><br><br><a href=relancebis.pl?action=rappel1&niveau=rappel1>1 rappel</a>&nbsp;&nbsp;<img src=http://intranet.dom/relance1.gif border=0 align=center>date de facture + 3 semaines<br></br>
	<a href=relancebis.pl?action=rappel2&niveau=rappel2>2 rappel</a>&nbsp;&nbsp;<img src=http://intranet.dom/relance2.gif border=0 align=center>date de facture + 5 semaines<br><br>
	<a href=relancebis.pl?action=rappel3&niveau=rappel3>3 rappel</a>&nbsp;&nbsp;<img src=http://intranet.dom/relance3.gif border=0 align=center>date de facture + 6 semaines<br><br>
	<a href=relancebis.pl?action=rappel4&niveau=rappel4>4 rappel</a>&nbsp;&nbsp;<img src=http://intranet.dom/relance4.gif border=0 align=center>date de facture + 8 semaines<br><br>
	<a href=relancebis.pl?action=rappel5&niveau=rappel5>5 rappel</a>&nbsp;&nbsp;<img src=http://intranet.dom/relance5.gif border=0 align=center>date de facture + 10 semaines<br>
	</center></html></body>
eof
}
sub sautepage{
