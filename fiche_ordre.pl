...
if ($action eq ""){
	print "\n<form action=fiche_ordre.pl method=POST  name=go><font size=+2>";
	...
	while (($pr_cd_pr,$pr_desi,$tr_prix,$ord_ordre)=$sth->fetchrow_array) {
		 print "<tr><td>$ord_ordre</td><td>$pr_cd_pr $pr_desi</td><td><input type=text name=prix$pr_cd_pr value=$tr_prix size=4></td><td><a href=?action=visusup&lot_nolot=$lot_nolot&produit=$pr_cd_pr>Sup</a></tr>\n";
	}
