...
if ($mois<6){$an_ref=$an_ref-1;}
$firstdate="$an_ref-04-15";
$trimestre=-1;
...
if (($action ne "")&&($trimestre==-1)){
	print " Erreur La date de fin doit être soit le dernier jour d'un trimestre";
	$action="";
}
 
$four="1260";
$date_du_jour=`/bin/date +%d'/'%m'/'%Y`;
@trimestre_tab=("Janvier-mars","Avril-Juin","Juillet-Septembre","Octobre-Decembre");	
if ($action eq ""){
   print "<h3>Facture Distrimarq Boutique</h3>";
	print "<form>";
	&form_hidden();
	print "<br> <br>Premiere date (incluse) <input id=\"datepicker\" type=text name=firstdate size=12 value='15/04/$an_ref' disabled>";
...
if ($action eq "sendmail"){
	...
	# &save("update dfc.facture_pub set date_mail=curdate()  where base='dfc' and mag='backwall distrimarq' and fournisseur='$four' and facture_no='$facture'","af");
}

if ($action eq "check"){
	&save("create temporary table back_tmp (marque varchar(30),qte int(8),montant decimal(8,2), primary key (marque))"); 
	print "Du $firstdate au $lastdate  trimestre $trimestre_tab[$trimestre] $an_ref<br>";
	$query="select distinct enh_document from cameshop.enso,cameshop.enthead,cameshop.produit  where  es_dt>='$firstdate' and es_dt<='$lastdate' and es_type=10 and es_cd_pr=produit.pr_cd_pr  and pr_four=1260 and es_no_do=enh_no";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($enh_document)=$sth->fetchrow_array){
			$query="select marque,sum(livb_qte_fac),sum(livb_qte_fac*livb_prix) from dfc.livraison_b,cameshop.produit_desi where livb_id='$enh_document' and livb_code=code group by marque";
	...
	print "Du $firstdate au $lastdate  trimestre $trimestre_tab[$trimestre] $an_ref<br>";
	$query="select distinct enh_document from corsica.enso,corsica.enthead,corsica.produit  where  es_dt>='$firstdate' and es_dt<='$lastdate' and es_type=10 and es_cd_pr=produit.pr_cd_pr  and pr_four=1260 and es_no_do=enh_no";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($enh_document)=$sth->fetchrow_array){
			$query="select marque,sum(livb_qte_fac),sum(livb_qte_fac*livb_prix) from dfc.livraison_b,corsica.produit_desi where livb_id='$enh_document' and livb_code=code group by marque";
			$sth2=$dbh->prepare($query);
			$sth2->execute();
			while (($marque,$qte,$montant)=$sth2->fetchrow_array){
				&save("insert ignore into back_tmp values ('$marque',0,0)");
	...
	print "<table cellspacing=10 cellpadding=10 border=1><tr><th>Marque</th><th>Achat</th><th>5% sell-in</th>";
	print "</tr>";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($marque,$qte,$montant)=$sth->fetchrow_array){
		if ($qte==0){next;}
		$sell=round(5*$montant/100);
...
if ($action eq "facture"){ 
	...
	# &save("insert ignore into facture_pub values ('dfc','backwall distrimarq','$four','','$facture',curdate(),0,'','','','')");
 	&facture_suite();
	...
	&total();
	 # &save("update dfc.facture_pub set montant='$total_sell' where base='dfc' and mag='backwall distrimarq' and fournisseur='$four' and facture_no='$facture'","af");
	$pdf->save();
	$fichier1=$fichier;
...
sub facture_suite{
	...
	if ($index==0){
		...
		# &save("update dfc.facture_pub set pdf='$fichier' where base='dfc' and mag='backwall distrimarq' and fournisseur='$four' and facture_no='$facture'","af");
	...
	$tete_text->translate( 20/mm, $ligne/mm );
	$tete_text->text("FACTURE N° $facture SPECIMEN DE TEST");
	$ligne-=5;
	$tete_text->font( $font{'Helvetica'}{'Roman'}, 12/pt );
	$tete_text->translate( 20/mm, $ligne/mm );
	$tete_text->text("$tva");
