...
%compta_idx = &get_index_multiple("comptamatiere-accise",1);            
open(FILE2,"/home/var/spool/uucppublic/comptamatiere-accise.txt");     
@compta_dat = <FILE2>;

%enso_idx = &get_index_multiple("enso",1);            
...
sub ligne{
	...
	if (($douane_ref>1000)&&($douane_ref<2000)){print "A:"}
		...
		if ($total_sortie>0){
			...
			if (($douane_ref>=746401)&&($douane_ref<751901)){ 
				print "IM4 ";
			}
			if (($douane_ref>=751901)&&($douane_ref<779721)){ 
				print "EX3 ";
			}
			if ($douane_ref>=779721){ 
				print "DOC111 ";
			}
