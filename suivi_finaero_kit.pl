...
if ($an_ref==2014){$date_ref_fin="2014-12-09";}
$option=$html->param("option");

$dbh->disconnect();
$dbh = DBI->connect("DBI:mysql:host=195.114.27.208:database=togo;","web","admin",{'RaiseError' => 1});
$base_client=180;
$base_client_code=180;
$base_client_nom="Asky";
$base_gestionnaire="Finaero";
$base_ville="Lome";
$base_pays="Togo";
$base_rep="togo.oasix";
$base_dev1="XOF";
...
if ($action eq "" ){
  ...
  print "Année : <input type=text name=an_ref value=2014>";
...
if ($action eq "go" ){
	print "<center>Suivi encaissement $an_ref <br>";
# 	&form_hidden();
# 	print "<input type=hidden name=an_ref value='$an_ref'>";
	...
	print " au ";
	if (&get("select datediff(curdate(),'$date_ref_fin')")<0){
	      print &today();
	}
	else
	{
...
# 		 print "<tr><td>$no:".$total{"EUR"}."</td></tr>";
	...
	for $devise (@tabdevise) {
		...
		print "<td  align=right><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=det&dev=$devise&an_ref=$an_ref>$val_ent</a></td>";
		...
		print "<td  align=right><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=det_rem&dev=$devise&an_ref=$an_ref>$val_sor</a></td>";
...
# 	print $query;
	...
	while (($no)=$sth->fetchrow_array){
		...
		if ($montant !=0){
			...
			print "<tr><td>$no</td><td>$date_bor</td><td align=right>$montant</td></tr>";
		}
		$total_gen+=$montant;
...
if ($action eq "det_rem"){
	...
	print "<table cellspacing=0 border=1>";
 	...
 	$query="select no,date_remise,montant,ref from bordereau where date_remise>='$date_ref_debut' and date_remise<'$date_ref_fin' and devise='$dev' and montant!=0";
	# print $query;
 	$sth=$dbh->prepare($query);
 	$sth->execute();
 	while (($no,$date_remise,$montant,$ref)=$sth->fetchrow_array){
		$cash=0;
		...
		if (grep /banque/i,$ref){$style="style=background-color:lightblue";$total_bq+=$montant;}
 		print "<tr $style><td>$no</td><td>$date_remise</td><td align=right>$montant</td><td align=right>$cash</td><td align=right>$ref</td></tr>";
 		$total_gen+=$montant;
		$total_cash+=$cash;
...
# 	}
	...
	print "<tr><td colspan=2><b>Total remise banque</b></td><td align=right><b>$total_bq</b></td></tr>";
