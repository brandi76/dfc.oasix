...
# 	}
	...
	{
		if ($navire eq "MEGA 2"){$num_navire=2;}
		if ($navire eq "REGINA"){$num_navire=8;}
		if ($navire eq "SMERALDA"){$num_navire=14;}
		if ($navire eq "VICTORIA"){$num_navire=6;}
		if ($navire eq "MEGA 4"){$num_navire=4;}
		if ($navire eq "MEGA 1"){$num_navire=1;}
		if ($navire eq "MEGA 3"){$num_navire=3;}
...
# 		while ($navire=$sth2->fetchrow_array){
			...
			$query="select vda_cd_pr,sum(vda_qte) from vendu_corsica_auto,produit where vda_navire='$num_navire' and vda_date>='$date2' and vda_date<='$date' and vda_cd_pr=pr_cd_pr and ((pr_type=1 or pr_type=5)&& pr_sup!=4 && pr_sup!=5) group by vda_cd_pr";
#  			print "$query";
			$sth=$dbh->prepare($query);
			$sth->execute();
