...
close(FILE2);
 ...
 %double_idx = &get_index_num("douane/2003/douable030605",0);            
 open(FILE2,"/home/var/spool/uucppublic/douane/2003/douable030605.txt");     # cwgestion double entrepot 
		...
		$sus_nouveau-=$qte_sortie_tiers;
                if ($double_idx{$sus_cd_pr} ne ""){
                      	($null,$dou_qte,$null,$dou_date)=split (/;/,$double_dat[$double_idx{$sus_cd_pr}]);
                       	if ($dou_date<1030506){$sus_qte=$dou_qte;}
             	}
...
foreach (@TABLE)
{
	...
	if ($type ne $type_titre){
		if ($pass ne 0){
			...
			print $verifn;
			print "</td></tr>";
			print "</table><br>entrepot douanier<h1>.</h1>";
