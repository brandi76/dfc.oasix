...
# $max+=100;
if ($jour>18){$max+=100;}
$fin=&get("select last_day(curdate())");
$debut=&get("select DATE_SUB(LAST_DAY(NOW()),INTERVAL DAY(LAST_DAY(NOW()))-1 DAY)");
...
print "Mois de ",&cal($mois-1);
# ," Numero d'entrepositaire agree:FR00116S0031<br>";
...
$query="select es_cd_pr,es_no_do,es_dt,es_qte,es_qte_en,es_type from enso where es_dt>'$debut' and es_dt<='$fin'";
$sth=$dbh->prepare($query);
$sth->execute();
while (($es_cd_pr,$es_no_do,$es_dt,$es_qte,$es_qte_en,$es_type)=$sth->fetchrow_array){
      
        # $es_type=6 sortie facture;
        if (($es_qte==0)&&($es_qte_en==0)){next;}
	...
	$sth2=$dbh->prepare($query);
	$sth2->execute();
#  	if ($vent_code==0){print "**** $es_cd_pr $es_qte $es_qte_en $pr_deg $pr_pdn****<br>";}
# 	
# 	if ($vent_code<1){next;}
...
$sth=$dbh->prepare($query);
$sth->execute();
print "<table border=1 cellspacing=0 style=\"font-size: 8pt;\">";
...
while (($so1_type)=$sth->fetchrow_array){
	...
	&color();
	print "<th colspan=2 bgcolor=$color><font size=-2>$so1_type $type_desi<br>";
