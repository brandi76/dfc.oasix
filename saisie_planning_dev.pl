...
if ($option eq "recopievol"){
	...
	for ($i=$fl_date+7;$i<$date_finjl;$i=$i+7){
		...
		&save("insert ignore into flyhead value ('$i','$fl_vol','$base_client','$fl_nbrot','$fl_troltype','0','0','0','1','0','$datesql')","chec");
		$nb_recopie++;
...
if ($option eq "modifleg"){
	$flb_datetr=&nb_jour_sql(&datepicker($flb_datetr));
	&save("replace into flybody value ('$fl_date','$param','$flb_rot','$flb_datetr','$flb_voltr','0','0','$flb_tridep','$flb_triret','0')","chec");
	$fl_apcode=&get("select fl_apcode from flyhead where fl_vol='$fl_vol' and fl_date='$fl_date'")+0;
	if ($fl_apcode >0){&maj_vol();}
}	
			...
			<form role="form">
				<fieldset>
					<div class="form-group">
						...
						<div class="input-group date form_datetime col-md-3" data-date="" data-date-format="dd MM yyyy - HH:ii p" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd-HH-mm"> 
...
print <<EOF;
			<div class="form-group">
				Date butoire:
				<div class="input-group date form_date" data-date="" data-date-format="dd MM yyyy - HH:ii p" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd-hh-mm" style=width:200px> 
...
EOF
				print "</td><td><input name=flb_tridep value='$flb_tridep' required</td><td><input name=flb_triret value='$flb_triret' required></td>";
				print "<td>";
				...
				print "<tr><td style=font-size:0.8em>Equipage $leg</td><td>$flb_voltr</td><td>$datetr</td><td>$flb_tridep</td><td>$flb_triret</td>";
				$disabled="";
				if ((($option eq "ajoutleg")&&($fl_vol eq $param))||($option eq "ajouttron")||($option eq "modiftron")){$disabled="disabled=\"disabled\"";}
				print "<td>";
				if (($flb_rot!=11)||($fl_apcode==0)){
...
print <<EOF;
							<div class="form-group">
						<div class="input-group date form_datetime" data-date="" data-date-format="dd MM yyyy" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd" style=width:200px> 
							<input class="form-control" type="text" value="$flb_datetr_sv" readonly>
							<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
						</div>
						<input type="hidden" id="dtp_input2" value="$flb_datetr_sv" name=flb_datetr />
