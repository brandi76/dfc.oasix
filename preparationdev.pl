...
##### >rm traitement de releve de marchandise
if (($action eq "rm")||($action eq "rms")) ### RELEVE DE mARCHANDISE
{
	if ($action eq "rm"){
	...
	$sth=$dbh->prepare("select ord_cd_pr,pr_desi,pr_type,pr_codebarre from ordre,produit where ord_cd_pr=pr_cd_pr and pr_sup!=1 and pr_sup!=4 and pr_sup!=2 and pr_type!=15 order by ord_ordre");
	$sth->execute();
 ...
 if ($action eq "ecart")
{
	...
	foreach (@liste) {
		($type,$nb)=split(/;/,$_);
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=1 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and ord_ordre>=180 and pr_type=1 order by ord_ordre","af");
		$normal=0+&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=1","af");
		if ($nombre == $normal){print "<td align=center>$nombre</td>";} else {print "<td align=center><b>$nombre</b>/$normal</td>";}
	}
	# mettre ici la gestion par tiroir	
# 	print "</tr><tr>";
# 	foreach (@liste) {
# 		($type,$nb)=split(/;/,$_);
# 		$query="select distinct tr_tiroir from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=1 order by tr_tiroir";
# 		$sth=$dbh->prepare($query);	
# 		$sth->execute();
# 		@listetir=();
# 		while (($tr_tiroir)=$sth->fetchrow_array){
# 			print "<td>$tr_tiroir</td>";
# 		}
# 	}	
	
	
	
	print "</tr><tr><td><b>Parfums femmes</b></td>";
	foreach (@liste) {
		($type,$nb)=split(/;/,$_);
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre<180 and pr_type=1 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and ord_ordre<180 and pr_type=1 order by ord_ordre","af");
		$normal=0+&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre<180 and pr_type=1","af");
		if ($nombre == $normal){print "<td align=center>$nombre</td>";}else{print "<td align=center><b>$nombre</b>/$normal</td>";}
	}	
	...
	foreach (@liste) {
		($type,$nb)=split(/;/,$_);
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=5 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and pr_type=5 order by ord_ordre","af");
		$normal=0+&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=5","af");
		if ($nombre == $normal){print "<td align=center>$nombre</td>";}else{print "<td align=center><b>$nombre</b>/$normal</td>";}
	}	
	print "</tr>";	
	...
	foreach (@liste) {
		($type,$nb)=split(/;/,$_);
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=4 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and pr_type=4 order by ord_ordre","af");
		$normal=0+&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=4","af");
		if ($nombre == $normal){print "<td align=center>$nombre</td>";}else{print "<td align=center><b>$nombre</b>/$normal</td>";}
	}	
	print "</tr>";	
	$sth=$dbh->prepare("select distinct pr_cd_pr,pr_desi from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ord_cd_pr=pr_cd_pr and pr_cd_pr!=800200 and pr_cd_pr!=230511 and pr_cd_pr!=230508 and pr_cd_pr!=230506 and pr_cd_pr!=120030 and pr_cd_pr!=800201  order by ord_ordre");
	$sth->execute();
	while (($pr_cd_pr,$pr_desi)=$sth->fetchrow_array){
		...
		foreach (@liste) {
			...
			$qteo=&get("select tr_qte/100 from trolley where tr_code=$type and tr_cd_pr=$pr_cd_pr","af")+0;
                        if ($qtem eq $qteo){print "<td align=center>$qteo</td>";}else{print "<td align=center><b>$qtem</b>/$qteo</td>";}
		}	
	}
 	$sth=$dbh->prepare("select distinct pr_cd_pr,pr_desi from produit,ordre where ord_cd_pr=pr_cd_pr and (pr_cd_pr=800200 or pr_cd_pr=230511 or pr_cd_pr=230508 or pr_cd_pr=230506 or pr_cd_pr=120030 or pr_cd_pr=800201)  order by ord_ordre");
	$sth->execute();
	while (($pr_cd_pr,$pr_desi)=$sth->fetchrow_array){
		print "<tr><td>$pr_desi</td>";
		foreach (@liste) {
			...
			$qteo=&get("select tr_qte/100 from trolley where tr_code=$type and tr_cd_pr=$pr_cd_pr","af")+0;
       			if ($qtem eq ""){$qtem=$qteo;}else{$qtem+=0;}
  	     		if ($qtem eq $qteo){print "<td align=center>$qteo</td>";}else{print "<td align=center><b>$qtem</b>/$qteo</td>";}
		}	
	}
 ...
 if ($action eq "ecartold")
{
	...
	@liste=();
	while (($fl_troltype,$nb)=$sth->fetchrow_array){
		# $nombre=&get("select count(*) from ecartrol,produit where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$fl_troltype and (pr_type=1 or pr_type=5)","af");
		$nombre=&get("select count(*) from ecartrol,produit where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$fl_troltype ","af");

		if ($nombre>0){push (@liste,"$fl_troltype;$nb");}
	}
	
	foreach (@liste) {
		($type,$nb)=split(/;/,$_);
		print "<b> ***** trolley type:$type nb:$nb ******</b><br>";
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=1 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and ord_ordre>=180 and pr_type=1 order by ord_ordre","af");
		print "<b>$type:Parfum Homme $nombre </b><br>";
		print "<table border=1 cellspacing=0>";
		$sth=$dbh->prepare("select pr_cd_pr,pr_desi,ecr_qte/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and ord_ordre>=180 and pr_type=1 order by ord_ordre");
		$sth->execute();
		while (($pr_cd_pr,$pr_desi,$qte)=$sth->fetchrow_array){
			print "<tr><td>$pr_cd_pr</td><td>$pr_desi</td><td>$qte</td></tr>";
		}
		print "</table>";
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre<180 and pr_type=1 and tr_ordre <180 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and ord_ordre<180 and pr_type=1 order by ord_ordre","af");
		print "<b>$type:Parfum Femme $nombre</b><br>";
		print "<table border=1 cellspacing=0>";
		$sth=$dbh->prepare("select pr_cd_pr,pr_desi,ecr_qte/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and ord_ordre<180 and pr_type=1 order by ord_ordre");
		$sth->execute();
		while (($pr_cd_pr,$pr_desi,$qte)=$sth->fetchrow_array){
		...
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and tr_ordre>=180 and pr_type=5 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and pr_type=5 order by ord_ordre","af");
		print "<b>$type:Comestique $nombre</b><br>";
		print "<table border=1 cellspacing=0>";
		$sth=$dbh->prepare("select pr_cd_pr,pr_desi,ecr_qte/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and pr_type=5 order by ord_ordre");
		$sth->execute();
		while (($pr_cd_pr,$pr_desi,$qte)=$sth->fetchrow_array){
			print "<tr><td>$pr_cd_pr</td><td>$pr_desi</td><td>$qte</td></tr>";
		}
		print "</table>";
		$nombre=&get("select sum(tr_qte)/100 from trolley,produit where tr_code=$type and tr_cd_pr=pr_cd_pr and tr_cd_pr=pr_cd_pr and pr_type=2 and tr_cd_pr not in (select ecr_cd_pr from ecartrol where ecr_cdtrol=$type)","af");
		$nombre+=&get("select sum(ecr_qte)/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and pr_type=2 order by ord_ordre","af");
		print "<b>$type:alcool $nombre</b><br>";
		print "<table border=1 cellspacing=0>";
		$sth=$dbh->prepare("select pr_cd_pr,pr_desi,ecr_qte/100 from ecartrol,produit,ordre where ecr_cd_pr=pr_cd_pr and ecr_cdtrol=$type and ord_cd_pr=pr_cd_pr and pr_type=2 order by ord_ordre");
		$sth->execute();
		while (($pr_cd_pr,$pr_desi,$qte)=$sth->fetchrow_array){
			print "<tr><td>$pr_cd_pr</td><td>$pr_desi</td><td>$qte</td></tr>";
		}
...
if (($action eq "rmprint")&&($today ne "")) ### ImPRESSION DU RELEVE DE mARCHANDISE
{
	...
	$sth=$dbh->prepare("select ord_cd_pr,pr_desi,pr_codebarre from ordre,produit where ord_cd_pr=pr_cd_pr and pr_sup!=1 and pr_sup!=4 and pr_sup!=2  and pr_type!=15 order by ord_ordre");
	$sth->execute();
	...
	while (($pr_cd_pr,$pr_desi,$pr_codebarre)=$sth->fetchrow_array){
		...
		if (($total!=0)||($flag==1)){
			if (($pr_cd_pr==130320)||($pr_cd_pr==120572)||($pr_cd_pr==122071)){
				print "</table><div id=saut></div><center><h1>$nodepart</h1><table border=1 cellspacing=0>";
				print "<tr bgcolor=#FFFF66><td>&nbsp;</td><td>&nbsp;</td>";
				print "<th>Retour</th><th>Depart</th><th>Sortir</th><th>Remettre</th><th>Dispo</th><th>cond.</th><th>plat</th><th>Carton</th><th>Detail</th><th>Check</th></tr>";
			}			
			...
			print "<tr><td>$pr_cd_pr <b>$digit_f</td><td><a href=fiche_produit.pl?pr_cd_pr=$pr_cd_pr&action=visu>$pr_desi</a></td>";
			$pr_stre+=0;
...
while (($liv_vol,$liv_date,$liv_nolot)=$sth->fetchrow_array){
	...
	if ($fl_apcode ne '0'){
			print "<td><a href=edite_appro.pl?lot=$liv_nolot>$fl_apcode</a></td><td><a href=lot.pl?$liv_nolot=on&lot_nolot=".int($liv_nolot/100)."&action=modiflot>plomb</a></td><td><a href=?action=etiquette&appro='$fl_apcode'&nodepart='$nodepart'>etiquette</a></td>";
	}
	else
	{
		if ($fl_nolot==0){
...
print "<a href=avisdepdev.pl?nodepart=$nodepart>Avis de départ et bon d'appro</a><br>";
# print "<a href=?action=tpe&nodepart=$nodepart>Tpe</a><br>";
# print "<a href=gere_ecart.pl?nodepart=$nodepart>Saisie des ecarts</a><br>";
# print "<a href=?action=relevecart&nodepart=$nodepart>Inventaire pour le controle de la saisie des ecarts</a>";

# print "<br><a href=edite_appro.pl>Réédition d'un seul bon d'appro</a>";
print "<br>";
print "<br>";
print "<a href=avisdepdev.pl?nodepart=$nodepart&printer=11>Avis de départ et bon d'appro sur imprimante de secours</a><br>";
print "<br>";
print "<br>Fin de page</br>";
}

## >relevecart inventaire des produits saisie en ecart
	if ($action eq "relevecart") 
{
	print "<center><a href=?>debut</a><h1>$nodepart Check saisie des ecarts</h1>";
	$query="select distinct ord_cd_pr,pr_desi,pr_codebarre from ordre,produit,trace_jour where ord_cd_pr=pr_cd_pr and ord_cd_pr=tjo_cd_pr and date(tjo_date)=curdate() and pr_sup!=1 and pr_sup!=4 and pr_sup!=2 and pr_cd_pr<100000000 order by ord_ordre";
	# print $query;
	$sth=$dbh->prepare($query);
	$sth->execute();
	print "<table border=1 cellspacing=0>";
...
sub print_etiq{
	...
	if ($fl_troltype==8){
		print "<font size=+5>Business<br>Galley 1 POS 125<div id=saut></div>VAB<br>Galley 2 POS 174<br><div id=saut></div>Vins et bieres<br>Galley 6 POS 626<br><div id=saut></div>Vins et bieres<br>Galley 6 POS 676<br><div id=saut></div></font>";
	}
