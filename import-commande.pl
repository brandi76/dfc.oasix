...
if ($action eq ""){
	print "<body><center><h1>IMPORTATION COMMANDE</h1><br>";
	print "<br> creation du fichier corsica  a partir de code produit qte<br>un produit peut apparaitre deux fois<br>";
...
if ($action eq "import"){
	...
	foreach (@tab){
		...
		if (($pr_cd_pr == 0)||($qte == 0)){next;}
                ...
                if ($pr_cd_pr <100000){
                	$pr_cd_pr=&get("select nep_codebarre from neptune where nep_cd_pr='$pr_cd_pr'");			 
		}
...
# -E creation du fichier corsica par copier coller en vue de son importation par commande_client.pl  (qte positive) via les commandes  03/2008