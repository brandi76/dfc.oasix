...
# 		}
	...
	while (($flb2_datetr,$flb2_rot,$flb2_voltr)=$sth2->fetchrow_array){
		...
		$date_sql=&julian($flb2_datetr,"YYYY-MM-DD");
		# maj de vol
		$query="replace into vol values ('$appro','$rot','$flb2_voltr','$date','','','','$trajet','$fl_cd_cl','','','','$fl_troltype','$flb2_datetr','0','$date_sql')";
		&execute();
		# maj caisse
	...
	$cree=0;
	if ($pastouche==0){
&save("insert ignore into ecartrol_arch select '$appro',ecr_cd_pr,ecr_qte from ecartrol where ecr_cdtrol='$fl_troltype'");
...
# 	my($sth)=$dbh->prepare($query);
# 	$sth->execute();
# 	while (($v_troltype,$nb)=$sth->fetchrow_array){
# 	  $query="select tr_cd_pr,tr_qte from trolley where tr_code=$v_troltype";
# 	  my($sth)=$dbh->prepare($query);
# 	  $sth->execute();
# 	  while (($v_troltype,$nb)=$sth->fetchrow_array){
# 	  
# 	&save("insert into pick  select curdate(),pr_cd_pr,pr_stvol/100 from produit where pr_stvol>0","aff");
# 	$query="select ret_cd_pr,sum(ret_retour) from non_sai,retoursql where ns_code=ret_code group by ret_cd_pr";
# 	my($sth)=$dbh->prepare($query);
# 	$sth->execute();
# 	while (($ret_cd_pr,$sum_retour)=$sth->fetchrow_array){
# 	  &save("update pick set pi_qte=pi_qte-$sum_retour where pi_cd_pr=$ret_cd_pr and pi_date=curdate()");
# 	}  
...
# 	$query="select gsl_troltype,count(*) from geslot where gsl_ind=3 group by gsl_troltype";
	...
	while (($troltype,$nb)=$sth->fetchrow_array){
		...
		while (($tr_cd_pr,$qte)=$sth2->fetchrow_array){
			$qte*=$nb;
	 	 	&save("select pi_qte from pick where pi_date=curdate() and pi_cd_pr='$tr_cd_pr'");
...
sub bon_appro{
	&cartouche_appro();
		...
		$query="select appro.*,pr_desi,pr_type,tr_tiroir,pr_pdb from appro,produit,trolley where ap_code='$gsl_apcode' and ap_cd_pr=pr_cd_pr and tr_code='$v_troltype' and ap_cd_pr=tr_cd_pr and ap_qte0>0 order by tr_tiroir,ap_ordre";
...
Depart   !Escale 1 ! Escale 2 ! Escale 3 ! Escale 4 ! Arrivée LFW ! asky     
";
		for ($i=1;$i<=$gsl_nbpb;$i++){
			$var="gsl_pb".$i;
			print "${$var}     !         !          !          !          !             !$br";
...
print " $at_type $depart*
*----------------------------------------------------------------------------*$br";
print "1ER EXEMPLAIRE POUR GESTIONNAIRE / 2E EX C/C ASKY  
------------------------------------------------------------------------------- 
VISA ASKY DEPART !VISA  GEST   DEPART   VISA RETOUR GEST     VISA ASKY RETOUR  ! 
                 !                     !          	   !                   ! 
                 !                     !                   !                   !
                 !                     !                   !                   !
                 !                     !                   !                   !
...
$cour_cfa=int($cour_cfa*$cour_euro/10)*10;
print "
...
Num d.enve  Num appro  date des vols  NOM+TRIGRAM C/C  ASKY NOM AGENT 
           !          !             !                 !                     ! 
           !          !             !                 !                     ! 
           !          !             !                 !                     ! 
           !          !             !                 !                     ! 
