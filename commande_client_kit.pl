...
if ($action eq ""){
	...
	print "<form>";
	&form_hidden();
	print "<table border=1 cellspacing=0><tr><th>Numero de commande<br><input type=text name=parnum size=4> <input type=submit class=bouton2 value=go></th><th>code client<br><input type text name=parclient size=4> <input type=submit class=bouton2 value=go></th><th>Commentaire</th><th>Date</th><th>Facture</th></tr>";
	...
	if ($parclient eq ""){
		...
		while (($ic2_no,$ic2_cd_cl,$ic2_com1,$ic2_com2,$ic2_fact,$ic2_date)=$sth->fetchrow_array){
			...
			print "<tr><td>$ic2_no</td><td>$ic2_cd_cl</td><td>$ic2_com1 $ic2_com2</td><td>$date</td><td>$ic2_fact</td><td>$nbref</td><td><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=validation&no_cde=$ic2_no>edite</a></td></tr>";
	...
	while (($ic2_no,$ic2_cd_cl,$ic2_com1,$ic2_com2,$ic2_fact,$ic2_date)=$sth->fetchrow_array){
		...
		$date=substr($ic2_date,5,2)."/".substr($ic2_date,3,2)."/".substr($ic2_date,1,2);
		print "<tr><td>$ic2_no</td><td>$ic2_cd_cl</td><td>$ic2_com1 $ic2_com2</td><td>$date</td><td>$ic2_fact</td><td><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=validation&no_cde=$ic2_no>edite</a></td></tr>";
	}
	print "<tr><td colspan=3><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=nouveau>Nouveau</a></td></tr>";
...
# 				print $query;
	...
	print "<font size=+2><b>Numéro de commande $no_cde</font><br><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=suptout&no_cde=$no_cde>suppression de la commande</a><br><a href=?action=supprod&no_cde=$no_cde>suppression des produits de la commande</a><form name=saisie><br>";
	# $query="select ic2_cd_cl,cl_nom,ic2_com1,ic2_fact from infococ2,client where ic2_no='$no_cde' and cl_cd_cl=ic2_cd_cl";
	# $sth = $dbh->prepare($query);
    	# $sth->execute;
	# ($cl_cd_cl,$cl_nom,$comment,$ic2_fact)=$sth->fetchrow_array; 
	...
	while (($pr_cd_pr,$pr_desi,$qte,$prix,$casse,$pr_type,$pr_prac)= $sth->fetchrow_array) {
		...
		print "<tr><td>$pr_cd_pr</td><td>$pr_desi</td><td>$qte</td><td>$prix</td><td>$casse</td><td><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=validation&produit=$pr_cd_pr&no_cde=$no_cde&qte=0>sup</a></td>";
		$nbligne++;
	...
	if ($ic2_fact==0){
		...
		$sth2->execute;
	    	...
	    	print "<br>\n";
		print "<br>qte <input type=text name=qte size=3 value=1>&nbsp;prix <input type=text name=prix size=5 value=auto>&nbsp;casse<input type=checkbox name=casse ";
		if ($cl_cd_cl==432){print " checked";}
		print "><br><br><br><br>";
	...
	print "<a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=bonprepnew&no_cde=$no_cde>Bon de preparation </a><br>";
# 	print "<a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=bon_de_livraison&no_cde=$no_cde>Bon de livraison</a><br>";
	print "<a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=facture&no_cde=$no_cde>Facture</a><br><br>";
# 	print "<a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&action=note&no_cde=$no_cde>Note de detail</a><br>";
	print "<a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet>Debut</a><br>";
	print "<input type=hidden name=no_cde value=$no_cde>";
	print "</form>";
}
=pod
...
if ($action eq "imputation_du_stock"){
	...
	while (($pr_cd_pr,$pr_desi,$qte)= $sth->fetchrow_array) {
		...
		$sth3=$dbh->prepare($query);
    		$sth3->execute;
    		$query="replace into enso values('$pr_cd_pr','$no_cde',curdate(),'$qte','0','5')";
		$sth3=$dbh->prepare($query);
    		$sth3->execute;
...
sub bonprepnew {
	$page=1;
	print "<h4>Bon de preparation de Commande</h4>";
	print "<br><b>Numéro de commande $no_cde<br>";
	print `date`."<br>";
	print "$cl_cd_cl $cl_nom $comment<br><br>";
	...
	$sth->execute;
	($cl_cd_cl,$cl_nom,$comment)=$sth->fetchrow_array; 
	$query="select coc_cd_pr,pr_desi,coc_qte/100,pr_four,coc_casse,pr_sup,pr_four from comcli,produit where coc_no='$no_cde' and coc_cd_pr=pr_cd_pr order by pr_four,pr_emb ";
	$sth=$dbh->prepare($query);
    	$sth->execute;
	while (($pr_cd_pr,$pr_desi,$coc_qte,$pr_four,$coc_casse,$pr_sup,$pr_four)=$sth->fetchrow_array){
	...
	print "<br><a href=?onglet=$onglet&sous_onglet=$sous_onglet&sous_sous_onglet=$sous_sous_onglet&no_cde=$no_cde&action=bonprepnew&option=sup><font color=red>Suppression des manquants</font></a>";
