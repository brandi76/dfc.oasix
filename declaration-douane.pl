...
open(VERIF,">/home/var/spool/uucppublic/verif.txt");
...
open(COMPTA,">/home/var/spool/uucppublic/comptamatiere-douane.txt");     # mouvement des produits
...
foreach(@suspen_dat){ 
	...
	if ($mvt_idx{$sus_cd_pr} ne ""){ # si des mouvements sont enregistres pour ce produit
		...
		foreach (@liste){
			...
			$qte_sortie+=$sortie;
			$qte_entree+=$entree;
			# si le flag est a 1 c'est que c'est un produit en double entrepot
			# si le numéro de douane est inférieur à 740000 c'est qu'il a eu des sorties en daa
...
print "<font size=+1><b><a name=ED>Entrepot Douanier</a></b></font><br><br><br>";
&recap();
print "<h1>.</h1>";
...
foreach (@TABLE)
{
	...
	if ($type ne $type_titre){
		if ($pass ne 0){
			...
			print $verifn;
			print "</td></tr>";
			print "</table>Valeur:$valeur<br>entrepot douanier<h1>.</h1>";
			$tableau[$type_titre]="$total_ancien;$total_ent;$total_so;$total_new;";
			$total_ancien=$total_ent=$total_so=$total_new=$valeur=0;
...
#	$qten_ancien+=600000;}
	...
	print "</font></td><td>";
#	 if ($code==222527){ # triche a enlever 
#	$qten_sortie+=600000;}
	...
	print "</td><td>";
	print "<font size=-1>";
	# du fait de la gestion multicode le stock nouveau t1 n'est pas le meme que le stock nouveau daa
	# la variable verif correspond à l'ecart entre les deux stock

	$verif=$qte_entree - $qte_sortie + $qte_ancien - $qte_new;
	$verifn=$qten_entree - $qten_sortie + $qten_ancien - $qten_new;

	$qte_new=$qte_entree - $qte_sortie + $qte_ancien;
#	if ($code==222527){ # triche
#	print &deci(22,2);
#	}
#	else {
...
#	}
	print "</font></td><td>";
	print VERIF "$pr_cd_prod;",&deci($qte_new,2),";\n";
	$qten_new=$qten_entree - $qten_sortie + $qten_ancien ;
	print $qten_new;
