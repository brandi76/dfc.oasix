...
sub bon_en_retard
{
	...
	$query="select v_code,v_date,v_troltype,v_dest from vol,etatap where v_date_jl <$dateref and at_code=v_code and at_etat=2 and v_rot=1";
...
sub calcul
{
	...
	$query="select pr_cd_pr,pr_stanc,pr_stre,pr_desi,pr_qte_comp,pr_deg,pr_douane,pr_pdn,pr_ventil from produit where pr_acquit=1";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($pr_cd_pr,$pr_stanc,$pr_stre,$pr_desi,$pr_qte_comp,$pr_deg,$pr_douane,$pr_pdn,$pr_ventil)=$sth->fetchrow_array){
