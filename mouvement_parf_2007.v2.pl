...
$query="select pr_cd_pr,pr_desi,pr_stre/100,pr_casse/100,pr_diff/100,pr_prac/100,pr_sup,pr_codebarre from produitdec where (pr_type=1 or pr_type=5) and pr_sup!=5";
$sth=$dbh->prepare($query);
$sth->execute();
while (($pr_cd_pr,$pr_desi,$pr_stre,$pr_casse,$pr_diff,$pr_prac,$pr_sup,$pr_codebarre)=$sth->fetchrow_array)
{
...
# entree
$query="select enb_cdpr,pr_desi,sum(enb_quantite/100),pr_prac/100,pr_prx_rev/100,pr_codebarre from entbody,produit,enthead where enb_cdpr=pr_cd_pr and enh_no=enb_no and (pr_type=1 or pr_type=5) and pr_sup!=5 and enh_date>13513 and enh_date<13879 and pr_desi not like 'teste%' group by enb_cdpr"; 
$sth=$dbh->prepare($query);
$sth->execute();
...
# livraison navire
$query="select pr_cd_pr,sum(coc_qte/100),pr_codebarre from infococ2,comcli,produit where ic2_cd_cl=500 and coc_in_pos=5 and coc_no=ic2_no and ic2_date>=1070100 and ic2_date<1071300 and coc_cd_pr=pr_cd_pr and coc_qte!=0 and pr_desi not like 'testeur%' and (pr_type=1 or pr_type=5) and pr_sup!=5 and pr_prac!=0 group by coc_cd_pr";
$sth=$dbh->prepare($query);
$sth->execute();
...
# vente avion
$query="select pr_cd_pr,sum(ro_qte)/100,pr_codebarre from rotation,vol,produit where ro_code=v_code and v_rot=1 and ro_cd_pr=pr_cd_pr and v_date_jl>='13514' and v_date_jl<='13879' and (pr_type=1 or pr_type=5) group by ro_cd_pr";
$sth=$dbh->prepare($query);
$sth->execute();
while (($pr_cd_pr,$qte,$pr_codebarre)=$sth->fetchrow_array){
	$pr_vente=100*$qte;
...
# client exterieur
$query="select pr_cd_pr,sum(coc_qte/100),pr_codebarre from infococ2,comcli,produit where ic2_cd_cl!=500 and coc_in_pos=5 and coc_no=ic2_no and ic2_date>=1070100 and ic2_date<1071300 and coc_cd_pr=pr_cd_pr and coc_qte!=0 and pr_desi not like 'testeur%' and (pr_type=1 or pr_type=5) and pr_sup!=5 group by coc_cd_pr";
$sth=$dbh->prepare($query);
$sth->execute();
while (($pr_cd_pr,$qte,$pr_codebarre)=$sth->fetchrow_array){
	$pr_ext=100*$qte;
...
$query="select infr_code from inforetsql,vol where infr_date>'2007-12-31' and infr_code=v_code and v_rot=1 and  FROM_UNIXTIME(v_date_jl*24*60*60,'%Y-%m-%d')<='2007-12-31'";
$sth=$dbh->prepare($query);
$sth->execute();
while (($v_code)=$sth->fetchrow_array){
	$query="select ro_cd_pr,sum(ro_qte)/100,pr_codebarre from rotation,produit where ro_code=$v_code and ro_cd_pr=pr_cd_pr and (pr_type=1 or pr_type=5) and pr_sup!=5 group by ro_cd_pr";
...
print "produit;stock0107;stock3107;entree;sortie;vente;exterieur;ecart;enlair0107,enlair312;ecart<br>";
