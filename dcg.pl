...
if ($jour>5){$max+=100;}  # remettre 18 au lieu de 5
...
print "<body alink=black vlink=black link=black><title>recap mensuel</title><body>";
$option=$html->param("option");
...
sub calcul
{
	...
	# $query="select pr_cd_pr,pr_stanc,pr_stre,pr_desi,pr_qte_comp,pr_deg,pr_douane,pr_pdn,pr_ventil,pr_acquit,pr_douane,pr_pdb from produit";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($pr_cd_pr,$pr_stanc,$pr_stre,$pr_desi,$pr_qte_comp,$pr_deg,$pr_douane,$pr_pdn,$pr_ventil,$pr_acquit,$pr_douane,$pr_pdb)=$sth->fetchrow_array){
		if ($pr_ventil<1){next;}
		...
		# if (($es_type==6) && ($es_dt==20071214)){next;}	
			...
			if ($pr_ventil==16){
				$sortie=$es_qte*$pr_pdb*100;
				$entree=$es_qte_en*$pr_pdb*100;
			}
	...
	if (($action eq "stre")||($action eq "stanc")||($action eq "entree")||($action eq "sortie")){ print "<tr><th>Total</th><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td align=right>$total_comp</td></tr>\n</table>";}	
}
...
{
		%stock_aux=&stock($pr_cd_pr,'',"");
		$stck_aux=$stock_aux{"stock"}+0;
		$query="select count(*) from produit where pr_cd_pr=$pr_cd_pr and (pr_type=1 or pr_type=5  or pr_ventil=15 or pr_ventil=16 or pr_cd_pr=1223935 or pr_cd_pr=120185 or pr_cd_pr=120170 or pr_cd_pr=120040 or pr_cd_pr=1223076 or pr_cd_pr=1221370 or pr_cd_pr=1222845 or pr_cd_pr=1220068 or pr_cd_pr=1225490 or pr_cd_pr=1225488 or pr_cd_pr=1222945 or pr_cd_pr=260615 or pr_cd_pr=1219970 or pr_cd_pr=120920 or pr_cd_pr=1222851 or pr_cd_pr=1221470 or pr_cd_pr=1222845 or pr_cd_pr=1225488 or pr_cd_pr=400095 or pr_cd_pr=1220700 or pr_cd_pr=1221086 or pr_cd_pr=1221082 or pr_cd_pr=198029 or pr_cd_pr=1220644 or pr_cd_pr=120054) and pr_cd_pr!=1231050 and pr_cd_pr!=1242613 ";
		...
		if ($res <=0) {$stck_aux="nil";}
			...
			if (($pr_ventil==6)||($pr_ventil==8)){ # alcool  # gallon
				$stock_aff=$stock_action/100;
			...
			print "$info_entree </td><td>$pr_douane</td><td>$stck_aux</td>";
			print "</tr>\n";
			$total_comp+=$comp;
}
