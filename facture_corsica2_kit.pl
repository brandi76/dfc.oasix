...
if ($action eq ""){
	...
	$query="select distinct livb_id,livh_four from livraison_b,livraison_h where livb_qte_ent>0 and livb_id=livh_id and livh_base='corsica' order by livh_id";
...
#   }
  $total=0;
  &save("replace into facture_corse2 value ('$facture','$fichier','$total_tva','$bl')");
