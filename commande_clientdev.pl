...
$perl="http://ibs.oasix.fr/cgi-bin/commande_clientdev.pl";
$html=new CGI;
print $html->header;
...
sub facture(){
	...
	$sth=$dbh->prepare($query);
	$sth->execute();
	($ic2_fact,$ic2_cd_cl)=$sth->fetchrow_array;
	$ic2_fact+=0;
	print "*$ic2_fact";
	if ($ic2_fact==0){ 
		#############################
		...
		while (($pr_cd_pr,$pr_desi,$qte,$casse)= $sth->fetchrow_array) {
			...
			$sth3=$dbh->prepare($query);
				...
				$pr_prac*=100;
	    		    	&save("update comcli set coc_casse=$pr_prac where coc_no='$no_cde' and coc_cd_pr='$pr_cd_pr'","aff");
		    	}
		}
...
=cut
	...
	print "<br><a href=http://ibs.oasix.fr/cgi-bin/commande_clientdev.pl?action=validation&no_cde=$no_cde>retour</a>";
