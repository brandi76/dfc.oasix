...
sub bon_appro{
	&cartouche_appro();
		...
		while (($ap_code,$ap_ordre,$ap_cd_pr,$ap_prix,$ap_qte0,$ap_cd_pos,$ap_cd_cl,$pr_desi,$pr_type,$tr_tiroir,$pr_pdb)=$sth4->fetchrow_array)
		{
			...
			print "!___! - !___! = !____ *$ap_prix E= ______! !______!$br";
		} # fin boucle appro
 		if ($total!=0){print " -->Nombre  de Produit tiroir $tiroir :$total$br";}
...
Depart   !Escale 1 ! Escale 2 ! Escale 3 ! Escale 4 ! Arrivée LFW ! camair/finaero     
";
		for ($i=1;$i<=$gsl_nbpb;$i++){
			$var="gsl_pb".$i;
			print "${$var}     !         !          !          !          !             !$br";
...
******************************************************************************
*$cl_nom CONTROLE QUALITE    No $gsl_apcode du edition $jour/$mois/$an*
*                                                                            *";
print "
*----------------------------------------------------------------------------*
*   Lot No    !           No VOL     DESTINATION        DATE      HEURE CHARG*
...
------------------------------------------------------------------------------- 
VISA ASKY DEPART !VISA FINAERO DEPART   VISA RETOUR FINAERO  VISA ASKY RETOUR  ! 
                 !                     !                   !                   !
                 !                     !                   !                   !
                 !                     !                   !                   !
                 !                     !                   !                   !
...
Nombre    XOF          Total                     Nombre    XAF          Total
----------------------------                     ----------------------------
!      !10 000 XOF !       !                    !       !10 000 XAF !       !
----------------------------                     ---------------------------- 		
!      ! 5 000 XOF !       !                    !       ! 5 000 XAF !       !
----------------------------                     ---------------------------- 		
!      ! 2 000 XOF !       !                    !       ! 2 000 XAF !       !
...
Nombre    USD          Total                     Nombre    EUR          Total
----------------------------                     ----------------------------
!       !   50 USD !       !                    !       !   100€    !       !
----------------------------                     ---------------------------- 		
!       !   20 USD !       !                    !       !    50€    !       !
