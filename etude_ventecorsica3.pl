...
&save("create temporary table produitsyl (pr_cd_pr bigint(20) NOT NULL)");

# (vdu_navire='MEGA 1' or vdu_navire='MEGA 2' or vdu_navire='MEGA 4')
...
for ($i=701;$i<713;$i++){
	&save("delete from produitsyl");
	$query="select vdu_cd_pr,sum(vdu_qte) as qte from vendu_corsica_mois where vdu_mois=$i and vdu_famille like 'PARFUMS' and (vdu_navire='MEGA 1' or vdu_navire='MEGA 2' or vdu_navire='MEGA 4') group by vdu_cd_pr order by qte desc limit $deb,$fin";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($pr_cd_pr,$qte)=$sth->fetchrow_array){
		&save("insert into produitsyl values ('$pr_cd_pr')");
	}
	print "$i ";
	$max=$max2=$total=0;
	$query="select vdu_qte from vendu_corsica_mois where vdu_famille='PARFUMS' and vdu_mois=$i and (vdu_navire='MEGA 1' or vdu_navire='MEGA 2' or vdu_navire='MEGA 4') and vdu_cd_pr in (select * from produitsyl) order by vdu_qte desc limit 3";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($qte)=$sth->fetchrow_array){
		print "$qte ";
		if (($max2==0)&&($max!=0)){$max2=$qte;}
		if ($max==0){$max=$qte;}
	}
	if (($max/$max2)>1.40){$max=$max2;}
	print " $max ";
	print "<br>";
	$rank=$deb."_".$fin;
	&save("insert into maxmois values ('1','$rank','$i','$max')","aff");
	# $max=int($max/4);
# 	print "$i $max <br>";
}
}
...
for ($i=701;$i<713;$i++){
	&save("delete from produitsyl");
	$query="select vdu_cd_pr,sum(vdu_qte) as qte from vendu_corsica_mois where vdu_mois=$i and vdu_famille like 'PARFUMS' and (vdu_navire!='MEGA 1' and vdu_navire!='MEGA 2' and vdu_navire!='MEGA 4') group by vdu_cd_pr order by qte desc limit 0,10";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($pr_cd_pr,$qte)=$sth->fetchrow_array){
		&save("insert into produitsyl values ('$pr_cd_pr')");
	...
	print "$i ";
	$max=$max2=$total=0;
	$query="select vdu_qte from vendu_corsica_mois where vdu_famille='PARFUMS' and vdu_mois=$i and (vdu_navire!='MEGA 1' and vdu_navire!='MEGA 2' and vdu_navire!='MEGA 4') and vdu_cd_pr in (select * from produitsyl) order by vdu_qte desc limit 3";
	$sth=$dbh->prepare($query);
	$sth->execute();
	while (($qte)=$sth->fetchrow_array){
		print "$qte ";
	...
	print "<br>";
	$rank=$deb."_".$fin;
	&save("insert into maxmois values ('2','$rank','$i','$max')");
