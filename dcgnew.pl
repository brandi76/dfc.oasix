...
sub calcul
{
	...
	$query="select pr_cd_pr,pr_stanc,pr_stre,pr_desi,pr_qte_comp,pr_deg,pr_douane,pr_pdn,npr_ventil,pr_acquit,pr_douane from produit,newventil where pr_cd_pr=npr_cd_pr";
	$sth=$dbh->prepare($query);
	$sth->execute();
...
sub titre_ancien_nouveau(){
		...
		if ($code==3){ # ABV VDL
			print "<caption><h3>Produits intermediaires</h3></caption><tr><th>Code</th><th>Désignation</th><th>Nb Bouteille</th><th>Litrage d\'une bouteille en cl</th><th>Total en hectolitre</th><th>Derniere entrée</th></tr>";
		}	
		if ($code==4){ # VDN
			print "<caption><h3>Vin doux naturel</h3></caption><tr><th>Code</th><th>Désignation</th><th>Nb Bouteille</th><th>Litrage d\'une bouteille en cl</th><th>Total en hectolitre</th><th>Derniere entrée</th></tr>";
		}	
		if ($code==6){ # ALCOOL
