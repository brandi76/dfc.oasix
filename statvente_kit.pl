...
{
  $base="camairco";
  $query="select pr_four,fo2_add from $base.produit,$base.ordre,$base.fournis where fo2_cd_fo=pr_four and pr_cd_pr=ord_cd_pr and pr_four=$four group by pr_four order by pr_four";
  # print ($query);
  $sth=$dbh->prepare($query);
  $sth->execute();
  while (($pr_four,$fo_nom)=$sth->fetchrow_array){
	  $query="select pr_cd_pr,pr_desi,floor(sum(ro_qte)/100) from $base.rotation,$base.vol,$base.produit where ro_code=v_code and v_date_sql>='$premiere' and v_date_sql<='$derniere' and v_rot=1 and ro_cd_pr=pr_cd_pr and pr_four='$pr_four' " ; 
	  print "$query";
	  $sth=$dbh->prepare($query);
	  $sth->execute();
...
=pod
	  ...
	  print "</tr>";
	  $query="select pr_cd_pr,pr_desi from $base.produit,$base.ordre where pr_cd_pr=ord_cd_pr and pr_four='$pr_four'";
	  $sth2=$dbh->prepare($query);
	  $sth2->execute();
