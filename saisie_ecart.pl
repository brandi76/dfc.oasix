...
if ($action eq ""){
  	...
  	print "<br><table border=1 cellspacing=0><caption><h3>Inversion</h3></caption><tr><th>Code produit en excedant</th><th>Code produit en deficit<t/h><th>qte</th></tr>";
	...
	print "<br><table><caption><h3>Casse</h3></caption><tr><th>Code produit</th><th>qte</th></tr>";
  	...
  	print "<tr><td><input type=text name=ca9 size=6></td><td><input type=text name=qtec9 size=3></td></tr>";
        print "</table>";
    	print "<br>\n<br><input type=hidden name=action value=go><input type=submit value= envoie></form></body>";
}
...
if ($action eq "go"){
	...
	for ($i=1;$i<10;$i++){
		...
		print "<tr><td>${ex.\"$i\"}</td><td>$pr_desi</td><td>+${qte.\"$i\"}</td>";
	...
	for ($i=1;$i<10;$i++){
		...
		&execute();			
		$query="replace into trace_jour values (now(),'5','$pr_cd_pr','$casse','','','')"; 			
		&execute();
		$pr_desi=&get("select pr_desi from produit where pr_cd_pr=${ca.\"$i\"}");
		%stock=&stock(${ex."$i"},'','');
		$pr_stre=$stock{"pr_stre"}+0;  # stock reel entrepot 
