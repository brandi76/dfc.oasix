...
# --------------------- fin gestion des mouvements par type ----------------
	...
	if ($sus_nouveau ==0){next;}
	if (($sus_famille ==3) && ($double_idx{$sus_cd_pr} ne "")){
                      	($null,$dou_qte,$dou_stre,$dou_date)=split (/;/,$double_dat[$double_idx{$sus_cd_pr}]);
                       	$sus_nouveau-=$dou_stre;
...
$tiers=2;
 &detail();
print "BIS FRANCE<br>";
print "<center><h2>INVENTAIRE ENTREPOT FISCAL</h2><br></center>";
&recap();
...
foreach (@TABLE)
{
	($type,$code,$qte_ancien,$qte_sortie,$qte_entree,$qte_new,$double)=split (/;/,$_);
	if ($type==10){next;}
	$qte_ancien+=0;
	$qte_sortie+=0;
	$qte_entree+=0;
	...
	if ($type ne $type_titre){
		if ($pass ne 0){
			...
			#print $verifn;
			print "</td></tr>";
			print "</table><br>entrepot fiscal<h1>.</h1>";
...
#	        print "</td><th colspan=2>ancien</td><th colspan=2>entree</td><th colspan=2>sortie</td><th colspan=2>nouveau</td><td>&nbsp;</td><td>&nbsp;</td></tr>";	
	        print "</td><th>stock<br>unité commerciale</td><th>stock<br>unité suplémentaire</td></tr>";	
...
print "<tr><td>&nbsp;</td><td><b>STOCK</td></tr>";
