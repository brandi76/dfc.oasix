...
$query="select distinct vdu_navire from vendu_corsica_mois where vdu_mois>=601 and vdu_mois<=612 and vdu_navire!='0'";
...
$query="select distinct vdu_mois from vendu_corsica_mois where vdu_mois >=601 and vdu_mois<=612 order by vdu_mois";
$sth=$dbh->prepare($query);
$sth->execute();
...
foreach $navire (@navire) {
	...
	foreach $mois (@mois) {
		...
		while (($pr_four,$pr_cd_pr,$pr_desi,$qte,$vendu,$achat)=$sth->fetchrow_array)
		{
			...
			$sth2->execute();
			($pr_prac,$pr_rem)=$sth2->fetchrow_array;
		...
		if ($totalv==0){print "<td align=right>&nbsp;</td>";}
			...
			print "<td align=right>$totalv<br>$marge%<br>$totalq</td>";
		}
		$totalvm+=$totalv;
		$totalam+=$totala;
		$totalqm+=$totalq;
	...
	print "<th align=right>$totalvm<br>$totalam<br>$marge%<br>$totalqm</th>";
	print "</tr>";
}
...
# 		print "$query<bR>";
	...
	$marge=int(($totalvm-$totalam)*10000/$totalvm)/100;
	print "<th align=right>$totalvm<br>$totalam<br>$marge%</th>";
	print "</tr>";
}
print "<tr><th>Total</th>";
foreach $mois (@mois) {
