...
#  	print "$query";
	...
	print "Cde no:$nocde<br>$four $fo2_add $date<br>";
  	print "<table cellspacing=0 border=1><tr><th>Ref four</th><th>code produit</th><th>produit</th><th>qte</th><th>Prix</th><th>check</th></tr>";
	while (($com2_no,$com2_cd_pr,$pr_desi,$com2_qte,$com2_date,$pr_refour,$com2_prac)=$sth->fetchrow_array){
		print "<tr><td>$pr_refour</td><td>$com2_cd_pr</td><td>$pr_desi</td><td align=right><input type=text name=qte$com2_cd_pr value='$com2_qte' size=5 style=text-align:right></td><td align=right>$com2_prac</td><td><input type=checkbox name=$com2_cd_pr></td></tr>";
	}
	print "</table>";
	print "<input type=hidden name=nocde value=$nocde>";
	print "<p style=background=pink>Désormais les entrées se font via le menu achat->entrée</p>";
# 	print "<br> <input type=submit value=\"Ok pour faire l'entree ?\">
	print "</form>";
...
if ($action eq "creer") {
	...
	($nocde)=$sth->fetchrow_array;
	 ...
	 &save("insert ignore into commande_info (com_no,date,user,etat) values ('$nocde',curdate(),'$user','0')");
	 &edite_commande();
	 }
}
...
sub page_pdf{
  ...
  if ($index==0){
    ...
    $tete_text->text("SAS au capital de 100000¤€ RCS PARIS 524 057049 00016 TVA FR 09524057049");
    $tete_text->fillcolor('black');
    $ligne=160;
    $tete_text->font( $font{'Helvetica'}{'Bold'}, 12/pt );
    $tete_text->translate( 20/mm, $ligne/mm );
...
sub fin_page_pdf{
	...
	# $tete_text->font( $font{'Helvetica'}{'bold'}, 12/pt );
	$tete_text->text("Nous vous demandons d’envoyer une liste de colisage des marchandises livrées et porter la destination finale sur chaque colis.");
	$pdf->save();
}	
...
sub page_xls{
	...
	$row++;
	$worksheet->write($row,$col,"1 Passage du grand cerf");
	$row++;
...
sub fin_page_xls{
	...
	$row+=5;
    $worksheet->write($row,$col,"SAS au capital de 100000 Euros RCS PARIS 524 057049 00016 TVA FR 09524057049");
	$workbook->close();
...
sub edite_commande {
	...
	$date_commande=&date_iso($date_commande);
#    	
	print "<div style=color:navy;;font-weight:bold;>DutyFree Concept<br>1 Passage du grand cerf<br />75002 PARIS <br />Tel 06 98 37 94 94 Email:supply_dfc\@dutyfreeconcept.com</div>";
	$query="select * from fournis where fo2_cd_fo='$four'";
	$sth=$dbh->prepare($query);
	$sth->execute();
	($fo2_cd_fo,$fo2_add,$fo2_telph,$fo2_fax,$fo2_contact,$fo2_identification,$fo2_delai,$fo2_transp,$fo2_livraison,$fo2_transport,$fo2_deb,$fo2_freq,$fo2_email)=$sth->fetchrow_array;
	...
	print "<div id=pied style=color:navy;font-weight:bold;position:absolute;bottom:10px;>SAS au capital de 100000¤€ RCS PARIS 524 057049 00016 TVA FR 09524057049</div>";
	# if ($action ne "double"){
