...
foreach $client (@bases_client){
    ...
    @liste=();
    &ratio();
    $query="select date,datediff(date,curdate()) from dfc.depart where date>curdate() order by date";
    $sth=$dbh->prepare($query);
    $sth->execute();
    while (($date,$delai)=$sth->fetchrow_array){
      @liste_four=();
      $query="select fo2_cd_fo,fo2_delai from fournis where fo2_delai<=$delai and fo2_delai>0";
      $sth3=$dbh->prepare($query);
      $sth3->execute();
      while (($four,$fo_delai)=$sth3->fetchrow_array){
	if ( grep /$four/,@liste){next;}
	$pass=0;
	...
	if ($first){
	  print "<strong> Export du $date dans $delai jours pour $client</strong><br>";
	  print "mag:$mag<br>\n";
