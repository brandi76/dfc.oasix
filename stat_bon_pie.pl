...
print "<center><form> <table border=1 cellspacing=0 cellpadding=10 style=font-size:9pt;><tr><th>Premier bon d'appro</th><th>Dernier bon d'appro</th><th>Destination(facultatif)</th><th>Trolley type (facultatif)<th>Famille</th>";
print "</tr> ";
	...
	while (($v_code)=$sth->fetchrow_array){
		...
		while (($ro_cd_pr,$ro_qte,$pr_type,$ap_prix)=$sth2->fetchrow_array){
			...
			if ($famille !=-1){ 
				...
				$pr_famille=$ro_cd_pr;
				$ap_qte0=&get("select ap_qte0 from appro where ap_code='$v_code'");
				if ($ro_qte==$ap_qte0){$rupture{$pr_famille}=$rupture{$pr_famille}+1;}else{$rupture{$pr_famille}=$rupture{$pr_famille}+0;}
				if ($ap_qte0==0){$manquant{$pr_famille}=$manquant{$pr_famille}+1;}else{$manquant{$pr_famille}=$manquant{$pr_famille}+0;}
			}
...
foreach $cle (sort(keys(%vente))){
	...
	else
	{
			...
			$desi="$desi $pour % rup:$rupture{$cle} manq:$manquant{$cle}";
	}
