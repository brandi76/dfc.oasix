...
$query="select distinct vdu_navire from vendu_corsica_mois where vdu_mois>=801 and vdu_mois<=812 and vdu_navire!='0'";
...
$query="select distinct vdu_mois from vendu_corsica_mois where vdu_mois >=801 and vdu_mois<=812 order by vdu_mois";
$sth=$dbh->prepare($query);
$sth->execute();
...
# 		print "$query<bR>";
		...
		while (($pr_four,$pr_cd_pr,$pr_desi,$qte,$vendu,$achat)=$sth->fetchrow_array)
		{
			#$famille=&get("select tva_ssfamille from corsica_tva where tva_refour='$pr_cd_pr'");
			#if ($famille eq "MAGAZINES/JEUX/GUIDES/ROM"){next;}
			if ($vendu==0){next;}
			$nep=0;
			&save("update produit set pr_prac=0 where pr_cd_pr='$pr_cd_pr'");
			#$pr_prac=&prac($pr_cd_pr);
			#if ($pr_prac==0){
			$pr_prac=$achat;
			$nep=1;
		...
		if ($totalv==0){print "<td align=right>&nbsp;</td>";}
			...
			$marge=int(($totalv-$totala)*10000/$totalv)/100;
			print "<td align=right>$totalv<br>$marge%<br>$totala</td>";
		}
		$totalvm+=$totalv;
		$totalam+=$totala;
		$totv{$mois}+=$totalv;
	...
	$marge=int(($totalvm-$totalam)*10000/$totalvm)/100;
	print "<th align=right>$totalvm<br>$marge%<br>$totalam</th>";
	print "</tr>";
}
print "<tr><th>Total</th>";
foreach $mois (@mois) {
