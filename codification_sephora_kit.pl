...
if ($action eq "match"){
	&save("update corsica.produit set pr_acquit='$ref' where pr_cd_pr=$pr_cd_pr","af");
	$action="marque";
}	

if ($action eq "marque"){
	$query="select corsica.produit.pr_cd_pr,pr_desi from corsica.produit,corsica.produit_desi,corsica.produit_plus where code=produit.pr_cd_pr and marque='$marque' and (pr_sup=0 or pr_sup=3) and pr_acquit=0 and produit.pr_cd_pr=produit_plus.pr_cd_pr and (pr_famille=1 or pr_famille=3) limit 1";
...
if ($action eq ""){
	...
	print "<h5>Marque</h5>";
	&form_hidden();
	$query="select distinct marque from corsica.produit_desi where code in (select es_cd_pr from corsica.enso)  order by marque";
	$pass=0;
	$sth=$dbh->prepare($query);
	$sth->execute();
	...
	while (($marque)=$sth->fetchrow_array){
		if ($marque eq ""){next;}
		$nb=&get("select count(*) from corsica.produit,corsica.produit_desi,corsica.produit_plus where code=produit.pr_cd_pr and marque='$marque' and (pr_sup=0 or pr_sup=3) and pr_acquit=0 and produit.pr_cd_pr=produit_plus.pr_cd_pr and (pr_famille=1 or pr_famille=3)");
		if ($nb==0){next;}
