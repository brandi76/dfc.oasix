print "<title>importation planning blue line</title>";
print "<div class=titre>Importation du planning</div><br>";
...
foreach $ligne (@ligne){
	...
	($vol,$date,$volrot,$daterot,$rot,$type,$dep,$arr,$hdep,$har)=split(/\t/,$ligne);
	($jour,$mois,$an)=split(/\//,$date);
	$datef=&nb_jour($jour,$mois,$an);
        $ref=$vol.$datef;
        if ($ref ne $tampon){
        	print "<hr>";
# 	        $save("replace into flyhead values ('$datef','$vol','10','2','10','0','0','0','0','0')");
		$pass=0;        
	}
        if ($type eq "A") {$index=10+$rot;} 
        else {
        	if ($pass==0) {
        		$index=21;
        	}
	...
	($jour,$mois,$an)=split(/\//,$daterot);
	$daterotf=&nb_jour($jour,$mois,$an);
	$hdep=~s/://;
 	$har=~s/://;
# 	$save("replace into flybody values ('$datef','$vol','$index','$daterotf','$volrot','$hdep','$har','$dep','$arr','0')");

 	print "$vol du $date tronçon $volrot $daterot $dep $arr<br>"; 
        $tampon=$ref;
}
sub null {	
	$nb=0;
...
# 		 print "*$mot*";
		if (((grep /200./,$mot)||(grep /JAN[0-9][0-9]/,$mot)||(grep /FE.[0-9][0-9]/,$mot)||(grep /MAR[0-9][0-9]/,$mot)||(grep /A.R[0-9][0-9]/,$mot)||(grep /MA.[0-9][0-9]/,$mot)||(grep /JU.[0-9][0-9]/,$mot)||(grep /JUIN[0-9][0-9]/,$mot)||(grep /AOU[0-9][0-9]/,$mot)||(grep /AUG[0-9][0-9]/,$mot)||(grep /SEP[0-9][0-9]/,$mot)||(grep /OCT[0-9][0-9]/,$mot)||(grep /NOV[0-9][0-9]/,$mot)||(grep /DEC[0-9][0-9]/,$mot)||(grep /[0-9]JAN$/,$mot)||(grep /[0-9]FE.$/,$mot)||(grep /[0-9]MAR$/,$mot)||(grep /[0-9]A.R$/,$mot)||(grep /[0-9]MA.$/,$mot)||(grep /[0-9]JU.$/,$mot)||(grep /[0-9]JU.$/,$mot)||(grep /[0-9]AOU$/,$mot)||(grep /[0-9]AUG$/,$mot)||(grep /[0-9]SEP$/,$mot)||(grep /[0-9]OCT$/,$mot)||(grep /[0-9]NOV$/,$mot)||(grep /[0-9]DEC$/,$mot))&&((! grep /\//,$mot)&&(! grep /CDG/,$ligne)&&(! grep /PAX/,$ligne))){
                        # print "*$mot*";
			$jour=substr($mot,0,2);
			$mois=substr($mot,2,3);
			$an=substr($mot,5,4);
...
sub magique
{
	...
	if (($v_cd_cl==545)&&($desi ne "Portugal")){$v_cd_cl=345;}
