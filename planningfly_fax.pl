...
if ($action eq "validmodif"){
	...
	# rotation supplementaire
		...
		($jour,$mois,$annee)=split(/\//,$datetr);
		$datetr=&nb_jour($jour,$mois,$annee);
...
sub table{
	...
	print  &julian($today,"");
	print  "</h1></div>\n";
	print  "<table border=1 cellspacing=0 cellpadding=0><tr><th>Compagnie</th><th>Vol</th><th>Trolley type</th><th>Tronçon</th><th>Validation</th></tr>";
	...
	while (($fl_date,$fl_vol,$fl_cd_cl,$fl_nbrot,$fl_troltype,$fl_apcode)=$sth->fetchrow_array){
		...
		while (($flb_tridep,$flb_triret,$flb_depart,$flb_arrivee,$flb_rot,$flb_voltr,$flb_datetr)=$sth2->fetchrow_array){
			...
			print "<td align=right><font>".&cal_heure($flb_depart,$decalage-1)."</td>";
			print "</td><td align=right>$flb_triret</td><td align=right><b>$flb_arrivee</td>";
			print "<td align=right><font>".&cal_heure($flb_arrivee,0-$decalage)."</td>";
...
sub vol{
	...
	print  &julian($today,"");
	print  "</h1>\n";
	print  "<form><table border=1 cellspacing=0 cellpadding=0><tr><th>Compagnie</th><th>Vol</th><th>Trolley type</th><th>Tronçon</th></tr>";
	$query="select fl_date,fl_vol,fl_cd_cl,fl_nbrot,fl_troltype from flyhead where fl_date=$date and fl_vol='$vol'";
	$sth=$dbh->prepare($query);
	$sth->execute();
	...
	while (($fl_date,$fl_vol,$fl_cd_cl,$fl_nbrot,$fl_troltype)=$sth->fetchrow_array){
		...
		print  $cl_nom;
		print  "</td>";
		print  "<td align=center><input type=text name=volhead value=$fl_vol size=8></td><td align=center><input type=text name=troltype value=$fl_troltype size=4></td><td><table border=1 cellspacing=0 width=100%>";
	...
	print  "</table><br>";
	print "<input type=submit class=bouton value=modifier>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Supprimer<Input type=checkbox name=sup><br><br>Ajouter <input type=text name=decalage value=7 size=2> jours copier <input type=checkbox name=copier>";
...
sub tetehtml{
	print "<body ><center><div class=ombre><font size=+5>Planning</font>";
}
