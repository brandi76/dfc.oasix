...
$query="select pr_cd_pr,pr_desi,pr_refour,pr_sup from produit where pr_four=1290 and pr_cd_pr<900000 order by pr_desi";
$sth=$dbh->prepare($query);
$sth->execute();
while (($pr_cd_pr,$pr_desi,$pr_refour,$pr_sup)=$sth->fetchrow_array){
  ...
  if (($check==0)&&($pr_sup!=3)){next;}
  print "<a href=http://dfc.oasix.fr/cgi-bin/kit_dfc.pl?onglet=&sous_onglet=0&sous_sous_onglet=0&prg=&achat=on&action=visu&pr_cd_pr=$pr_cd_pr target=_blank>$pr_cd_pr</a> $pr_desi $pr_refour  :$check<br>";
  $nb++;
}  
print "<br>$nb";
