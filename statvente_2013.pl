...
print "<body><center><h1>2013</h1>";

if ($action eq "") {
	print "<strong>annee</strong> <br>";
	print "<form>";
  	...
  	print "</select><br><input type=text name=fourmanu size=4><br><input type=hidden name=action value=go><br><input type=submit value='envoie'></form>";
...
$query="select v_cd_cl from vol where floor(v_date%100)=13 group by v_cd_cl order by v_cd_cl" ;
...
$nbclient=$#client+2;
$query="select pr_four,fo2_add from produit,ordre,fournis where fo2_cd_fo=pr_four and pr_cd_pr=ord_cd_pr and pr_four=$four group by pr_four order by pr_four";
# print ($query);
$sth3=$dbh->prepare($query);
$sth3->execute();
while (($pr_four,$fo_nom)=$sth3->fetchrow_array){
	$query="select floor(sum(ro_qte)/100) from rotation,vol,produit where ro_code=v_code and floor(v_date%100)=13 and v_rot=1 and ro_cd_pr=pr_cd_pr and pr_four='$pr_four' " ; # and (v_dest like 'LYS%' or v_dest like 'MRS%')
	...
	print "</tr>";
	$query="select pr_cd_pr,pr_desi from produit,ordre where pr_cd_pr=ord_cd_pr and pr_four='$pr_four'";
	$sth2=$dbh->prepare($query);
	$sth2->execute();
	while (($pr_cd_pr,$pr_desi)=$sth2->fetchrow_array){
		if (($action eq "filtre")&&($html->param("$pr_cd_pr") ne "on")){next;}
		if (&get("select floor(sum(ro_qte)/100) from rotation,vol,produit where ro_code=v_code and v_rot=1 and floor(v_date%100)=13 and ro_cd_pr=pr_cd_pr and pr_cd_pr='$pr_cd_pr'","af")+0==0){next;}
		print "<tr><td width=30%>$pr_cd_pr $pr_desi</td>";
		$total=0;
		$totalgen=0;
		for ($i=113;$i<1313;$i=$i+100){
...
if ($action eq "gomois")
{
	...
	while (($pr_four,$fo_nom)=$sth3->fetchrow_array){
		...
		$totalf=0;
		print "<h3>$pr_four $fo_nom</h3><br>";
		print "<h2>$tab_mois[$mois-1] 2013</h2><br>";
		print "<table cellspacing=0 border=1>";
		print "<tr bgcolor=#efefef><th>&nbsp;</th>";
		for ($i=0;$i<=$#client;$i++){
