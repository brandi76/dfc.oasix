...
$query="select es_cd_pr,es_no_do,es_dt,es_qte,es_qte_en,es_type from enso where es_type!=5";
...
while (($es_cd_pr,$es_no_do,$es_dt,$es_qte,$es_qte_en,$es_type)=$sth->fetchrow_array){
	...
	if (($vent_code<6)||(($vent_code>=10)&&($vent_code<=19)&&($vent_code!=15)&&($vent_code!=17))){
		$es_qte=$es_qte*$pr_pdn;
		$es_qte_en=$es_qte_en*$pr_pdn;
		}
...
$sth=$dbh->prepare($query);
$sth->execute();
print "<table border=1 cellspacing=0><tr><th>No de ligne</th><th>Date</th><th>Client</th><th>Document</th>";
$color="";
while (($so1_type)=$sth->fetchrow_array){
	...
	&color();
	print "<th colspan=2 bgcolor=$color>$type_desi<br>";
	$query="select vent_ndp from ventil_sql where vent_code=$so1_type";
	$sth2=$dbh->prepare($query);
	$sth2->execute();
