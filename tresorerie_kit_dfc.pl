	...
	<form role="form">
				<fieldset>
					<div class="form-group">
						<label for="dtp_input2" class="control-label">Premier Mois</label>
						...
						<input type="hidden" id="dtp_input2" value="" name=premiere /><br/>
						
						<label for="dtp_input3" class="control-label">Dernier Mois</label>
						<div class="input-group date form_date col-md-3" data-date="" data-date-format="MM yyyy" data-link-field="dtp_input3" data-link-format="yyyy-mm"> 
							<input class="form-control" size="16" type="text" value="" readonly>
							<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
						</div>
						<input type="hidden" id="dtp_input3" value="" name=derniere /><br/>
					</div>
				</fieldset>
...
$premiere=$html->param("premiere");
$derniere=$html->param("derniere");
($premier_an,$premier_mois)=split(/-/,$premiere);
($dernier_an,$dernier_mois)=split(/-/,$derniere);
...
if ($action eq "rentree"){
	...
	$an=$premier_an;
	$mois=$premier_mois+0;
	while (($mois<=$dernier_mois)||($an!=$dernier_an)){
		$montant=$html->param("${an}_${mois}");
		&save("replace into rentree value ('$base','$an','$mois','$montant')","");
		$mois++;
...
&form_hidden();
print "<input type=text name=ajust value=$ajust size=10> <input type=hidden name=premiere value=$premiere><input type=hidden name=derniere value=$derniere><input type=submit value=maj></form></td>";
$an=$premier_an;
$mois=$premier_mois+0;
...
foreach $client (@base_afrique){
	...
	while (($mois<=$dernier_mois)||($an!=$dernier_an)){
		...
		print "<td align=right><input name=${an}_${mois} value=$val size=2></td>";
		$mois++;
		if ($mois==13){$mois=1;$an++;}
	}
	print "</tr>";	
   print "<input type=hidden name=premiere value=$premiere><input type=hidden name=derniere value=$derniere>";
	print "</form>";
