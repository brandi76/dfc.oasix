...
";
	...
	while (($ap_code,$ap_ordre,$ap_cd_pr,$ap_prix,$ap_qte0,$ap_cd_pos,$ap_cd_cl,$pr_desi,$pr_type,$tr_tiroir,$pr_pdb)=$sth4->fetchrow_array)
	{
		...
		# detail pochon bijoux
		if (($ap_cd_pr==800200)&&($ap_qte0>0)){
			$query="select appro.*,pr_desi,pr_type,pr_pdb from appro,produit where ap_code='$gsl_apcode' and ap_cd_pr=pr_cd_pr and ap_ordre >=3500 and ap_ordre<3600 order by ap_ordre";
			$sth5=$dbh->prepare($query);
			$sth5->execute();
			while (($ap_code,$ap_ordre,$ap_cd_pr,$ap_prix,$ap_qte0,$ap_cd_pos,$ap_cd_cl,$pr_desi,$pr_type,$pr_pdb)=$sth5->fetchrow_array)
			{
				$pr_desi=&taillefixen($pr_desi,25);
				$total+=int($ap_qte0/100);
				$ap_qte0=&taillefixen($ap_qte0/100,3);
...
Le reste doit obligatoirement etre remis dans le dossier.
 ...
 TOTAL!              EU !             EU !              EU !          EU !
      --------------------------------------------------------------------
                                         ...
                                         MONTANT CAISSE    :";
#if ($v_cd_cl==333){ print "
#					 FOND DE CAISSE    : 50 euros";}
print "
