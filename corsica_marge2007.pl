...
$query="select distinct vdu_navire from vendu_corsica_mois where vdu_mois>=701 and vdu_mois<=712 and vdu_navire!='0'";
...
$query="select distinct vdu_mois from vendu_corsica_mois where vdu_mois >=701 and vdu_mois<=712 order by vdu_mois";
$sth=$dbh->prepare($query);
$sth->execute();
