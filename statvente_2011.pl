...
$query="select v_cd_cl from vol where floor(v_date%100)=11 group by v_cd_cl order by v_cd_cl" ;
...
while (($pr_four,$fo_nom)=$sth3->fetchrow_array){
	$query="select floor(sum(ro_qte)/100) from rotation,vol,produit where ro_code=v_code and floor(v_date%100)=11 and v_rot=1 and ro_cd_pr=pr_cd_pr and pr_four='$pr_four' " ; # and (v_dest like 'LYS%' or v_dest like 'MRS%')
	...
	while (($pr_cd_pr,$pr_desi)=$sth2->fetchrow_array){
		if (&get("select floor(sum(ro_qte)/100) from rotation,vol,produit where ro_code=v_code and v_rot=1 and floor(v_date%100)=11 and ro_cd_pr=pr_cd_pr and pr_cd_pr='$pr_cd_pr'","af")+0==0){next;}
		print "<tr><td width=30%>$pr_cd_pr $pr_desi</td>";
		$total=0;
		$totalgen=0;
		for ($i=111;$i<1311;$i=$i+100){
