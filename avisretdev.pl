...
$mois=`/bin/date '+%m'`;
$an="20".`/bin/date '+%y'`;
print "<center><h1>Avis de Retour</h1><br><form>Date d'enlevement: Jour <input type=text name=jour value=$jour> Mois <input type=text name=mois value=$mois> Annee <input type=text name=annee value=$an><br>Heure (HH):<input type=text name=heure value=8><br>";
...
$query="select gsl_nolot,gsl_dtret,gsl_novol,gsl_hrret,gsl_trajet,gsl_desi,gsl_apcode from geslot where (gsl_ind=3) and ((gsl_dtvol<'$today') or (gsl_dtvol='$today' and gsl_hrret<'$heure'*100)) order by gsl_nolot";

# print "<td>$query</td>";
$sth=$dbh->prepare($query);
$sth->execute();
$i=0;
while (($gsl_nolot,$gsl_dtret,$gsl_novol,$gsl_hrret,$gsl_trajet,$gsl_desi,$gsl_apcode)=$sth->fetchrow_array){
	$i++;
	...
	print  "</td>";
	print  "<td><b>$cl_trilot $gsl_nolot</td>";
...
pour le retour de notre camion</pre><table cellspacing=0 border=1><tr><th>No du lot</th><th>No bon</th><th>Depart no</th><th>No de vol</th><th>Dest</th><th>Conteneur</th><th>Alcool</th><th>Tabac</th></tr>";
for ($i=1;$i<=$nb;$i++){
	$check=$html->param("check$i");
	if ($check eq "on"){
		$appro=$html->param("appro$i");
		$query="select gsl_nolot,gsl_trajet,gsl_novol,gsl_desi,gsl_alc/100,gsl_tab/100,gsl_nb_cont,gsl_nodep from geslot where gsl_apcode=$appro";
		$sth=$dbh->prepare($query);
		$sth->execute();
		($gsl_nolot,$gsl_trajet,$gsl_novol,$gsl_desi,$gsl_alc,$gsl_tab,$gsl_nb_cont,$gsl_nodep)=$sth->fetchrow_array;
		print "<tr><td>$gsl_nolot</td><td>";		
		print "$appro</td><td align=right>";		
		print "$gsl_nodep</td><td>";		
...
print "</table><br>Nombre de conteneur:$nbcont<br>";
print "Note: Les quantités indiqués sont celles misent au départ de notre entrepôt<br><pre><br>Nous vous en remercions, et vous prions d'agreer, Messieurs, nos sinceres salutations";
 }
