...
while (($code,$rot)=$sth->fetchrow_array){
	$ca_recettes="";
	$query="select ca_papi,ca_papi from caissesql where ca_code='$code' and ca_rot='$rot'";
...
foreach (@index){
	...
	if ($nom ne $nomtampon){
		...
		print "<tr><th>Appro</th><th>Vol</th><th>Rotation</th><th>Date</th><th>Equipage</th><th>Stim</th><th>Nb pnc</th><th>Part</th></tr>";
