...
&cree_produit_tmp();
# $query="select * from facture_pub where facture_mag=marque='VERSACE'";
$facture_run=$html->param("facture_run");
$query="select * from facture_pub where no_facture=$facture_run";
$sth=$dbh->prepare($query);
$sth->execute();
($base,$mag,$four,$marque,$facture_mag,$date_mag,$montant_mag)=$sth->fetchrow_array;
...
sub create_pdf{  
	...
	$date_du_jour=&date_iso($date_mag);
	$index=0;
	$query = "select code,pr_desi,visuel,pub,visuelprix,pubprix,marque,desi_pub from $base.mag,produit_tmp where pr_cd_pr=code and (pub=1 or visuel=1) and mag='$mag' and pr_four='$four' and marque='$marque' order by pr_desi";
	$marqueindex=0;
	# print "$query<br>";
	...
	if ($ok){
		&total();
		$pdf->save();
		print "$base $mag $facture_mag $montant_mag*$total_bug<a href=http://dfc.oasix.fr/doc/fac_bug.pdf>file</a><br>";
	}
}
...
sub facture_suite{
  ...
  if ($index==0){
    ...
    $file="/var/www/dfc.oasix/doc/fac_bug.pdf";
  ...
  $tete_text->translate( 20/mm, $ligne/mm );
  $tete_text->text("FACTURE N° $facture_run");
  $ligne-=5;
  $tete_text->font( $font{'Helvetica'}{'Roman'}, 12/pt );
  $tete_text->translate( 20/mm, $ligne/mm );
